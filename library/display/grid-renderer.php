...
class UpFrontGridRenderer {
	...
	private function step_2_build_rows() {
		...
		foreach ( $this->blocks as $block_id => $block ) {
			...
			if ( $range_beginning && upfront_in_numeric_range($block['position']['top'], $range_beginning, $range_end) ) {
	...
	private function step_3_construct_columns() {
		...
		foreach ( $this->blocks as $block_id => $block ) {
			...
			if ( $sub_columns ) {
				...
				foreach ( $this->columns[$column_id] as $block_or_sub_column )
					upfront_remove_from_array($this->blocks_in_sub_columns, $block_or_sub_column);
		...
		private function step_3a_extract_sub_columns($origin_block_id) {
			...
			foreach ( $match_row_ids as $match_block_id => $match_row_id ) {
				...
				while ( $current_match = current($this->rows[$match_row_id]) ) {
					...
					if ( $current_match == $match_block_id ) {
						...
						$neighbors = upfront_array_key_neighbors($this->rows[$match_row_id], key($this->rows[$match_row_id]));
						...
						if ( isset($bad_matches[$match_block_id]) )
							upfront_remove_from_array($matches, $match_block_id);
			...
			foreach ( $matches as $match_id ) {
				...
				if ( isset($bad_matches[$match_id]) )
					upfront_remove_from_array($matches, $match_id);
		...
		private function step_3b_remove_block_from_columns($block_id_to_remove) {
			...
			foreach ( $this->columns as $column_id => $column_blocks ) {
				...
				if ( in_array($block_id_to_remove, $column_blocks) ) {
					...
					upfront_remove_from_array($this->columns[$column_id], $block_id_to_remove);
		...
		private function step_3c_modify_rows_for_sub_column_above_origin($origin_block, $first_block, array $sub_column_blocks) {
			...
			upfront_array_insert($this->rows[$first_block_row_id], array($origin_block['id']), $first_block_position_in_row);				

			foreach ( $sub_column_blocks_above_origin as $block_id )
				$this->step_3d_remove_block_from_rows($block_id);

			upfront_remove_from_array($this->rows[$origin_block_row_id], $origin_block['id']);
		...
		private function step_3d_remove_block_from_rows($block_id_to_remove) {
			...
			foreach ( $this->rows as $row_id => $row_blocks ) {
				...
				if ( in_array($block_id_to_remove, $row_blocks) ) {
					...
					upfront_remove_from_array($this->rows[$row_id], $block_id_to_remove);
				...
				foreach ( $column_content as $block_or_sub_index => $block_id_or_sub_content ) {
					...
					if ( !is_array($block_id_or_sub_content) ) {
						...
						/* If block CSS class bubbling is enabled then add that to row and column */
							...
							if ( upfront_get('css-classes-bubble', $block_settings, false) === true && upfront_get('css-classes', $block_settings) ) {


								$current_row['classes'] = array_merge($current_row['classes'], explode(' ' , upfront_get('css-classes', $block_settings)));
								$current_column['classes'] = array_merge($current_column['classes'], explode(' ' , upfront_get('css-classes', $block_settings)));
						...
						foreach ( $block_id_or_sub_content as $sub_block_id ) {
							...
							/* If block CSS class bubbling is enabled then add that to row and column and sub column */
								$block_settings = upfront_get('settings', $this->blocks[ $sub_block_id ]);

								if ( upfront_get('css-classes-bubble', $block_settings, false, true) === true && upfront_get('css-classes', $block_settings) ) {

									$current_row['classes'] = array_merge($current_row['classes'], explode(' ' , upfront_get('css-classes', $block_settings)));
									$current_column['classes'] = array_merge($current_column['classes'], explode(' ' , upfront_get('css-classes', $block_settings)));
									$current_sub_column['classes'] = array_merge($current_sub_column['classes'], explode(' ' , upfront_get('css-classes', $block_settings)));
	...
	public function render_grid() {
		...
		do_action('upfront_grid_container_open', $this->wrapper);

		foreach ( $this->finalized_layout as $row_index => $row ) {

			echo '<section class="' . implode(' ',  array_unique(array_filter($row['classes']))) . '">';

				do_action('upfront_block_row_open', $this->wrapper);

				foreach ( $row['columns'] as $column_index => $column ) {

					echo '<section class="' . implode(' ', array_unique(array_filter($column['classes']))) . '">';

					do_action('upfront_block_column_open', $this->wrapper);

					foreach ( $column['contents'] as $index => $block_or_sub_column ) {

						if ( upfront_get('type', $block_or_sub_column) == 'block' ) {

							UpFrontBlocks::display_block(upfront_get('block', $block_or_sub_column), 'grid-renderer');

						} elseif ( upfront_get('type', $block_or_sub_column) == 'sub-column' ) {

							echo '<section class="' . implode(' ', array_unique(array_filter($block_or_sub_column['classes']))) . '">';

							do_action('upfront_block_sub_column_open', $this->wrapper);

							foreach ( $block_or_sub_column['blocks'] as $sub_block ){								
								UpFrontBlocks::display_block($sub_block, 'grid-renderer');
							}

							do_action('upfront_block_sub_column_column', $this->wrapper);
					...
					do_action('upfront_block_column_close', $this->wrapper);

					echo '</section>';

				}

				do_action('upfront_block_row_close', $this->wrapper);

			echo '</section>';

		}		

		do_action('upfront_grid_container_close', $this->wrapper);
