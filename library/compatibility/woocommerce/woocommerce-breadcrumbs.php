<?php
add_filter('upfront_breadcrumbs', 'upfront_mp_breadcrumbs');
function upfront_mp_breadcrumbs($breadcrumbs) {

	/* Product Archive Page */
	if ( is_post_type_archive('product') && get_option('page_on_front') !== mp_get_page_id('shop') ) {

		$shop_page_id = mp_get_page_id('shop');

		$shop_name = $shop_page_id ? get_the_title($shop_page_id) : ucwords(get_option('woocommerce_shop_slug'));

		if ( is_search() ) {

			$breadcrumbs[] = __('Search results for &ldquo;', 'upfront') . get_search_query();
	...
	if ( is_tax('product_tag') ) {
		...
		$breadcrumbs[] = __('Products tagged &ldquo;', 'upfront') . single_term_title('', false) . _x('&rdquo;', 'endquote', 'upfront');
