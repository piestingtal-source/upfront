<?php
class UpFrontWebFontsLoader {


	public static function init() {

		add_action( 'wp', array( __CLASS__, 'enqueue_webfont_api_for_design_editor' ) );
		add_action( 'upfront_stylesheets', array( __CLASS__, 'google_fonts_stylesheet' ), 1 );
		add_action( 'upfront_flush_cache', array( __CLASS__, 'flush_cache' ) );
	...
	public static function enqueue_webfont_api_for_design_editor() {
		...
		wp_enqueue_script( 'webfont', upfront_format_url_ssl( 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ) );
	...
	public static function google_fonts_stylesheet() {
		...
		if ( ! $webfonts_in_use || !is_array(upfront_get('google', $webfonts_in_use)) )
			return;
		...
		if(strlen($fonts) > 0 ){
			...
			$font_display = UpFrontOption::get('google-fonts-display', false, 'swap');

			/**
			 * Preload Google Schriftarten
			 */
			if( UpFrontOption::get('google-fonts-preload', false, false) ){
			...
			if(UpFrontOption::get('load-google-fonts-asynchronously')){

				wp_enqueue_script('google-fonts-async', upfront_url() . '/library/media/js/google-fonts-asynchronously.js', array('jquery'), false, true);
				...
				$stylesheet_url = '//fonts.googleapis.com/css?display=' . $font_display . '&family=' . $fonts ;
				echo "<link rel='stylesheet' id='upfront-google-fonts' href='$stylesheet_url' type='text/css' media='all' />\n";
			}
	...
	/* End Google Web Schriftarten */


	public static function get_fonts_in_use() {

		/* If cache exists then use it */
		$cache = get_transient( 'pu_webfont_cache_template_' . UpFrontOption::$current_skin );
		...
		return get_transient( 'pu_webfont_cache_template_' . UpFrontOption::$current_skin );
	...
	public static function cache() {
		...
		$raw_webfonts 		= self::pluck_webfonts(UpFrontElementsData::get_all_elements());
		...
		return set_transient( 'pu_webfont_cache_template_' . UpFrontOption::$current_skin, $sorted_webfonts );
	...
	public static function flush_cache() {
		...
		return delete_transient( 'pu_webfont_cache_template_' . UpFrontOption::$current_skin );
