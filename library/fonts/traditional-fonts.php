<?php
class UpFrontTraditionalFonts extends UpFrontWebFontProvider {
	...
	public function query_fonts($sortby) {
		...
		foreach ( UpFrontFonts::get_fonts() as $font_id => $font_name )
			$fonts[] = array(
				'id' => $font_id,
				'name' => $font_name,
				'stack' => UpFrontFonts::get_stack($font_id) 
			);
...
class UpFrontFonts {
	...
	public static function get_fonts() {
		...
		return apply_filters('upfront_fonts', self::$fonts); 
	...
	public static function get_stack($font_id) {
		...
		return upfront_get($font_id, apply_filters('upfront_fonts_stacks', self::$font_stacks));
	...
	public static function register_font(array $args) {
		...
		if ( !isset($id) || !isset($stack) || !isset($name) )
			return new WP_Error('pu_fonts_register_font_invalid_args', __('To register a font, the argument array must include an "id", "stack", and "name".', 'upfront'), $args);
