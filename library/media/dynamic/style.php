<?php
class UpFrontDynamicStyle {


	static function design_editor() {

		/* Action used for registering elements */
		do_action('upfront_dynamic_style_design_editor_init');

		$elements = UpFrontElementsData::get_all_elements();
		...
		foreach ( $elements as $element_id => $element_options ) {
			...
			$element 	= UpFrontElementAPI::get_element($element_id);
			...
			if($element_id == 'wrapper' && isset($element_options['mirroring'])){
				...
				foreach ($element_options['mirroring'] as $orginal_wrapper_id => $target_wrapper_id) {
					...
					$target_wrapper_data = UpFrontWrappersData::get_wrapper($target_wrapper);
			...
			if ( isset($element_options['properties']) ) {
				$return .= UpFrontElementProperties::output_css($selector, self::filter_nudging_properties($element_options['properties'], $element));
			}
			...
			if ( isset($element_options['special-element-layout']) && is_array($element_options['special-element-layout']) ) {
				...
				foreach ( $element_options['special-element-layout'] as $layout => $layout_properties ) {
					...
					if ( UpFrontLayout::is_customized($layout) ) {
						$selector_prefix = 'body.layout-using-' . str_replace( UpFrontLayout::$sep, '-', $layout ) . ' ';
					} else {
						$selector_prefix = 'body.layout-' . str_replace( UpFrontLayout::$sep, '-', $layout ) . ' ';
					}
					...
					$return .= UpFrontElementProperties::output_css($layout_element_selector, self::filter_nudging_properties($layout_properties, $element));
			...
			if ( isset($element_options['special-element-instance']) && is_array($element_options['special-element-instance']) ) {
				...
				foreach ( $element_options['special-element-instance'] as $instance => $instance_properties ) {
					...
					$return .= UpFrontElementProperties::output_css($instance_selector, self::filter_nudging_properties($instance_properties, $element));
			...
			if ( isset($element_options['special-element-state']) && is_array($element_options['special-element-state']) ) {
				...
				foreach ( $element_options['special-element-state'] as $state => $state_properties ) {
					...
					$return .= UpFrontElementProperties::output_css($state_info['selector'], self::filter_nudging_properties($state_properties, $element));
	...
	static function wrapper() {
		...
		$layout_id = upfront_get('layout-in-use');
		$wrappers = UpFrontWrappersData::get_wrappers_by_layout($layout_id);
		...
		/* Default Container Margins */
			if ( upfront_get('file') == 've-iframe-grid-dynamic' && upfront_get('visual-editor-open') ) {
				...
				$return .= UpFrontElementProperties::output_css('div.wrapper', array(
					'margin-top' => UpFrontElementsData::get_property('wrapper', 'margin-top', UpFrontWrappers::$default_wrapper_margin_top, 'structure'),
					'margin-bottom' => UpFrontElementsData::get_property('wrapper', 'margin-bottom', UpFrontWrappers::$default_wrapper_margin_bottom, 'structure'),
					'padding-top' => UpFrontElementsData::get_property('wrapper', 'padding-top', null, 'structure'),
					'padding-right' => UpFrontElementsData::get_property('wrapper', 'padding-right', null, 'structure'),
					'padding-bottom' => UpFrontElementsData::get_property('wrapper', 'padding-bottom', null, 'structure'),
					'padding-left' => UpFrontElementsData::get_property('wrapper', 'padding-left', null, 'structure')
				));
		...
		foreach ( $wrappers as $wrapper_id => $wrapper ) {
			...
			$wrapper_grid_width = UpFrontWrappers::get_grid_width($wrapper);
			...
			if ( upfront_get('file') == 've-iframe-grid-dynamic' && upfront_get('visual-editor-open') ) {
				$wrapper_selector = 'div#wrapper-' . UpFrontWrappers::format_wrapper_id($wrapper['original-id']);
			} else {
			...
			/* Fluid Container */
					...
					/* Gitter Container */
						/* Fixed Gitter */
							if ( !(upfront_get('fluid', $wrapper_settings, false, true) && upfront_get('fluid-grid', $wrapper_settings, false, true)) ) {
					...
					/* Gitter */
						if ( upfront_get('file') != 've-iframe-grid-dynamic' || !upfront_get('visual-editor-open') ) {
							...
							if ( UpFrontResponsiveGrid::is_enabled() || ( upfront_get( 'fluid', $wrapper_settings, false, true ) && upfront_get( 'fluid-grid', $wrapper_settings, false, true ) ) ) {
			...
			/* Both Fixed and Fluid: Margin in Gitter Mode */
				if ( upfront_get('file') == 've-iframe-grid-dynamic' && upfront_get('visual-editor-open') ) {

					$wrapper_instance_id = 'wrapper-' . UpFrontWrappers::format_wrapper_id($wrapper['original-id']);

					$return .= UpFrontElementProperties::output_css($wrapper_selector, array(
						'margin-top' => UpFrontElementsData::get_special_element_property('wrapper', 'instance', $wrapper_instance_id, 'margin-top', null, 'structure'),
						'margin-bottom' => UpFrontElementsData::get_special_element_property('wrapper', 'instance', $wrapper_instance_id, 'margin-bottom', null, 'structure'),
						'padding-top' => UpFrontElementsData::get_special_element_property('wrapper', 'instance', $wrapper_instance_id, 'padding-top', null, 'structure'),
						'padding-right' => UpFrontElementsData::get_special_element_property('wrapper', 'instance', $wrapper_instance_id, 'padding-right', null, 'structure'),
						'padding-bottom' => UpFrontElementsData::get_special_element_property('wrapper', 'instance', $wrapper_instance_id, 'padding-bottom', null, 'structure'),
						'padding-left' => UpFrontElementsData::get_special_element_property('wrapper', 'instance', $wrapper_instance_id, 'padding-left', null, 'structure')
					));

				}

			/* Responsive Break Points */
				if ( !UpFrontRoute::is_visual_editor_iframe('grid') && !(upfront_get( 'file' ) == 've-iframe-grid-dynamic') ) {

					$responsive_options = upfront_get( 'responsive-wrapper-options', $wrapper_settings, array() );
					...
					if ( $options ) {
						...
						foreach ( $options as $option ) {
							...
							$breakpoint = upfront_fix_data_type( upfront_get_search( 'breakpoint', $option, 'off' ) );
							$max_width  = upfront_fix_data_type( upfront_get_search( 'max-width', $option, '' ) );


							if ( $max_width && $breakpoint == 'custom' )
								$breakpoint = $max_width;

							$breakpoint_min_max = upfront_fix_data_type( upfront_get_search( 'breakpoint-min-or-max', $option, 'max' ) );
							$stretch            = upfront_fix_data_type( upfront_get_search( 'stretch', $option, false ) );
							$auto_center        = upfront_fix_data_type( upfront_get_search( 'auto-center', $option, false ) );
							$hide_wrapper       = upfront_fix_data_type( upfront_get_search( 'hide-wrapper', $option, false ) );
							...
							if ( $hide_wrapper ){
								...
								if ( upfront_get('visual-editor-open') === 'true' && UpFrontOption::get('show-hidden-wrappers-on-design-mode', false, false) ) {
		...
		return $return;
			...
			static function fixed_grid(array $wrapper) {
				...
				if ( $potential_wrapper_mirror = UpFrontWrappersData::get_wrapper_mirror($wrapper_settings) )
					$wrapper_settings = $potential_wrapper_mirror;

				$grid_number = UpFrontWrappers::get_columns($wrapper);

				$column_width = UpFrontWrappers::get_column_width($wrapper);
				$gutter_width = UpFrontWrappers::get_gutter_width($wrapper);
				...
				/* Create a flag keeping this same Gitter CSS from being outputted */
			...
			static function responsive_grid(array $wrapper) {
				...
				/* Render the Gitter into arrays to see if sub column CSS will be needed */
					if ( $wrapper_mirror = UpFrontWrappersData::get_wrapper_mirror( $wrapper['id'] ) ) {
						$wrapper_blocks = UpFrontBlocksData::get_blocks_by_wrapper( $wrapper_mirror['layout'], $wrapper_mirror['id'] );
					} else {
						$wrapper_blocks = UpFrontBlocksData::get_blocks_by_wrapper( upfront_get( 'layout-in-use' ), $wrapper['original-id'] );
					}

					$wrapper_rendered = new UpFrontGridRenderer($wrapper_blocks, $wrapper_settings);
			...
			static function block_heights() {
				...
				foreach ( $blocks as $block ) {
					...
					/* If the block is mirrored then change the selector */
						if ( $mirrored_block_id = UpFrontBlocksData::is_block_mirrored($block) )
					...
					if ( upfront_get('fixed-height', upfront_get($block['type'], $block_types), false) !== true )
						$return .= $selector . ' { min-height: ' . $block['dimensions']['height'] . 'px; }';
					else
						$return .= $selector . ' { height: ' . $block['dimensions']['height'] . 'px; }';

					$responsive_options = upfront_get('responsive-options', $block['settings'], array());
					...
					if($options) {
						...
						foreach ($options as $option) {
							...
							/* Responsive CSS - some magic to make the columns work with the smartphone setting */
							$breakpoint = upfront_fix_data_type(upfront_get('blocks-breakpoint', $option, 'off'));

							if( !$breakpoint )
								continue;

							$max_width = upfront_fix_data_type(upfront_get('max-width', $option, ''));

							if($max_width && $breakpoint == 'custom')
								$breakpoint = $max_width . 'px';

							$disable_block_height = upfront_fix_data_type(upfront_get( 'disable-block-height', $option, false));
							$mobile_auto_center = upfront_fix_data_type(upfront_get( 'mobile-center-elements', $option, false));
							$breakpoint_min_max = upfront_fix_data_type(upfront_get( 'breakpoint-min-or-max', $option, 'max'));

							$fixed_height = upfront_get('fixed-height', upfront_get($block['type'], $block_types));

							/* Griddify Lists */
							$griddify_lists = upfront_fix_data_type(upfront_get( 'griddify-lists', $option, false));

							$hide_block = upfront_fix_data_type(upfront_get('hide-block', $option, false));
	...
	static function live_css() {
		...
		if ( upfront_get('visual-editor-open') )
			return null;

		return UpFrontSkinOption::get( 'live-css', false, null, false, false );
