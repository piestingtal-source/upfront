<?php
class UpFrontElementsData_Upgrade34 {
	...
	public static function get_all_elements() {
		...
		$option_groups = get_option( 'upfront_option_groups' );

		//Pull out only the design editor groups.  Since upfront_option_groups uses true for every value
		...
		foreach ( $design_editor_groups as $design_editor_group ) {
			...
			$group = get_option( 'upfront_option_group_' . $design_editor_group );
		...
		if ( is_array( $default_data ) ) {
			$elements = upfront_array_merge_recursive_simple( $default_data, $elements );
		}
	...
	public static function get_element_properties( $element_id ) {
		...
		if ( ! isset( UpFrontElementAPI::$elements[ $element_id ] ) ) {
			return new WP_Error( 'element_not_registered', __( 'The element ID is not registered.', 'upfront' ), $element_id );
		}

		//Set vars up and get stuff from database
		$group = UpFrontElementAPI::$elements[ $element_id ]['group'];
		$element = UpFrontOption::get( $element_id, 'design-editor-group-' . $group, array( 'properties' => array() ) );
	...
	public static function get_special_element_properties( $element_id, $se_type, $se_meta ) {
		...
		$element = UpFrontOption::get( $element_id, 'design-editor-group-' . $group, array(
	...
	public static function get_property( $element_id, $property_id, $default = null ) {
		...
		if ( $properties !== null && ! is_wp_error( $properties ) && isset( $properties[ $property_id ] ) ) {
			return upfront_fix_data_type( $properties[ $property_id ] );
		} else {
			return $default;
		}
	...
	public static function get_inherited_property( $element_id, $property_id, $default = null ) {
		...
		$inherit_location = UpFrontElementAPI::get_inherit_location( $element_id );
	...
	public static function set_property( $element_id, $property_id, $value, $forced_group = false ) {
		...
		//Send it back to DB
		UpFrontOption::set( $element_id, $element, 'design-editor-group-' . $group );
	...
	public static function delete_element( $element_id ) {
		...
		//Delete the element
		UpFrontOption::delete( $element_id, 'design-editor-group-' . $group );
	...
	public static function delete_all() {
		...
		$option_groups = get_option( 'upfront_option_groups', array() );
		...
		foreach ( $option_groups as $group_name => $group_name_bool_unused ) {
			...
			update_option( 'upfront_option_group_' . $group_name, array() );
	...
	public static function get_default_data() {
		...
		return apply_filters( 'upfront_element_data_defaults', $upfront_default_element_data );
...
class UpFrontLayoutOption_Upgrade34 {
	...
	public static function init() {
		...
		if ( upfront_get( 'preview' ) && UpFrontCapabilities::can_user_visually_edit() ) {

			UpFrontOption::$group_suffix = '_preview';
			UpFrontLayoutOption_Upgrade34::$group_suffix = '_preview';
	...
	public static function get( $layout = false, $option = null, $group_name = false, $default = null ) {
		...
		if ( ! $layout ) {
			$layout = UpFrontLayout::get_current();
		}

		//Format layout ID
		$layout = self::format_layout_id( $layout );

		$options = get_option( 'upfront_layout_options_' . str_replace( '-', '_', $layout ) . self::$group_suffix );

		if ( self::$group_suffix && ! $options ) {
			$options = get_option( 'upfront_layout_options_' . str_replace( '-', '_', $layout ) );
		}
		...
		$data = upfront_fix_data_type( $options[ $group_name ][ $option ] );
	...
	public static function set( $layout = false, $option = null, $value = null, $group_name = false ) {
		...
		$options = get_option( 'upfront_layout_options_' . $layout );

		//Get layout options catalog
		$catalog = get_option( 'upfront_layout_options_catalog' );
		...
		//Send data to DB
		update_option( 'upfront_layout_options_' . $layout . self::$group_suffix, $options );

		if ( ! self::$group_suffix ) {
			update_option( 'upfront_layout_options_catalog', $catalog );
		}
	...
	public static function delete( $layout, $option = null, $group_name = false ) {
		...
		if ( count( $options ) === 0 && ! self::$group_suffix ) {
			...
			$catalog = array_diff( get_option( 'upfront_layout_options_catalog' ), $removal );

			delete_option( 'upfront_layout_options_' . $layout );
			update_option( 'upfront_layout_options_catalog', $catalog );
	...
	public static function delete_all_from_layout( $layout ) {
		...
		$catalog = array_diff( get_option( 'upfront_layout_options_catalog' ), array( $layout ) );
...
class UpFrontBlocksData_Upgrade34 {
	...
	protected static function schema_blocks_by_id( $use_cache = false ) {
		...
		if ( $use_cache && $cached = upfront_get( 'blocks-by-id', self::$schema_cache ) ) {
			return $cached;
		}

		/* Retrieve the option from DB */
		$blocks_by_id = UpFrontOption::get( 'blocks-by-id', 'blocks', array() );
	...
	protected static function schema_blocks_by_type() {
		...
		/* Retrieve the option from DB */
		$blocks_by_type = UpFrontOption::get( 'blocks-by-type', 'blocks', array() );
	...
	protected static function schema_blocks_by_layout() {
		...
		/* Retrieve the option from DB */
		$blocks_by_layout = UpFrontOption::get( 'blocks-by-layout', 'blocks', array() );
	...
	protected static function schema_layout_blocks( $layout_id ) {
		...
		/* Retrieve the option from DB */
		$layout_blocks = UpFrontLayoutOption_Upgrade34::get( $layout_id, 'blocks', false, array() );
	...
	public static function add_block( $layout_id, $args ) {
		...
		//Update database
		UpFrontLayoutOption_Upgrade34::set( $layout_id, 'blocks', $layout_blocks );

		UpFrontOption::set( 'blocks-by-type', $blocks_by_type, 'blocks' );
		UpFrontOption::set( 'blocks-by-id', $blocks_by_id, 'blocks' );
		UpFrontOption::set( 'blocks-by-layout', $blocks_by_layout, 'blocks' );
	...
	public static function get_block( $block, $use_mirrored = false ) {
		...
		if ( is_array( $block ) ) {
			...
			if ( ! isset( $block['id'] ) && ! upfront_get( 'new', $block, false ) ) {
				return null;
			}
			...
			$layout_blocks = self::get_blocks_by_layout( upfront_get( 'layout', $blocks_by_id[ $block ] ) );
	...
	public static function get_blocks_by_layout( $layout_id, $include_design_editor_instances = false ) {
		...
		foreach ( $layout_blocks as $block_id => $block ) {
			...
			if ( $include_design_editor_instances ) {
				...
				$block_element = UpFrontElementAPI::get_element( 'block-' . $layout_blocks[ $block_id ]['type'] );

				/* Set up styling array */
				$layout_blocks[ $block_id ]['styling'] = array();

				/* Get block instance styling */
				$block_instance_properties = UpFrontElementsData::get_special_element_properties( array(
				...
				foreach ( upfront_get( 'children', $block_element, array() ) as $block_element_sub_element ) {
					...
					$sub_element_instance_properties = UpFrontElementsData::get_special_element_properties( array(
					...
					if ( ! empty( $block_element_sub_element['states'] ) && is_array( $block_element_sub_element['states'] ) ) {
						...
						foreach ( $block_element_sub_element['states'] as $instance_state_id => $instance_state_info ) {
							...
							$instance_state_properties = UpFrontElementsData::get_special_element_properties( array(
								'element' => $block_element_sub_element['id'],
								'se_type' => 'instance',
	...
	public static function get_blocks_by_wrapper( $layout_id, $wrapper_id ) {
		...
		foreach ( $layout_blocks as $block_id => $block ) {
			...
			if ( upfront_get( 'wrapper', $block, UpFrontWrappers::$default_wrapper_id ) === $wrapper_id ) {
	...
	public static function get_block_name( $block ) {
		...
		$default_name = UpFrontBlocks::block_type_nice( $block['type'] ) . ' #' . $block['id'];

		return upfront_get( 'alias', $block['settings'], $default_name );
	...
	public static function get_block_width( $block ) {
		...
		$block_grid_width = upfront_get( 'width', $block['dimensions'], null );

		if ( $block_grid_width === null ) {
			return null;
		}

		/* Fetch the wrapper that way we can get its Gitter settings */
		$wrapper = UpFrontWrappers::get_wrapper( upfront_get( 'wrapper', $block, 'wrapper-default' ) );

		return ( $block_grid_width * ( upfront_get( 'column-width', $wrapper ) + upfront_get( 'gutter-width', $wrapper ) ) ) - upfront_get( 'gutter-width', $wrapper );
	...
	public static function get_block_height( $block ) {
		...
		$block_grid_height = upfront_get( 'height', $block['dimensions'], null );
	...
	public static function get_block_setting( $block, $setting, $default = null ) {
		...
		return upfront_fix_data_type( $block['settings'][ $setting ] );
	...
	public static function is_block_mirrored( $block, $return_block_id = false ) {
		...
		if ( $block && $mirrored_block_id = upfront_get( 'mirror-block', $block['settings'] ) ) {
			...
			if ( upfront_get( 'type', $mirrored_block ) != upfront_get( 'type', $block ) ) {
				return false;
			}

			/* Make sure that the mirrored block isn't mirroring another block */
			$possible_mirror_of_mirror = upfront_get( 'mirror-block', $mirrored_block['settings'] );

			if ( $possible_mirror_of_mirror && $mirror_of_mirror_block = self::get_block( $possible_mirror_of_mirror ) ) {
				if ( upfront_get( 'type', $mirror_of_mirror_block ) == upfront_get( 'type', $mirrored_block ) ) {
					return false;
				}
			}
