    ...
    */
        ...
        $.fn.quicksearch = function(target, opt) {
                ...
                options = $.extend({
                    ...
                    testQuery: function(query, txt, _row) {
                        for (var i = 0; i < query.length; i +=
                            1)
                            if (txt.indexOf(query[i]) === -1) return false;
                        return true
                    }
                }, opt);
            this.go = function() {
            ...
            this.results = function(bool) {
                if (typeof options.noResults === "string" && options.noResults !== "")
                    if (bool) $(options.noResults).hide();
                    else $(options.noResults).show();
                return this
            };
            this.loader = function(bool) { if (typeof options.loader === "string" && options.loader !== "") bool ? $(options.loader).show() : $(options.loader).hide(); return this };
            this.cache = function() {
                jq_results = $(target);
            ...
            this.trigger = function() {
                ...
                timeout = window.setTimeout(function() { e.go() },
                    options.delay);
                return this
            };
            this.cache();
    ...
    function fontBrowserObj(browser) {
        ...
        this.initQuickSearch = function(context) {
            ...
            var quicksearch = context.find('.fonts-filter').quicksearch('#' + id + ' .fonts-list ul li', {
                ...
                testQuery: function(query, txt, _row) {
                    return query.test(txt.replace('Der schnelle Braunfuchs springt über den faulen Hund.', '').trim());
                }
            });
        ...
        this.initPreview = function(context) {
            ...
            previewHtml = $('<div class="font-preview-overlay" style="display:none;">' +
                ...
                '<header>' +
                '<h4></h4>' +
                '<p><i class="icon-edit">&nbsp;</i><strong>Klicke irgendwo</strong> im Vorschau-Text zum Bearbeiten und Hinzufügen Deines eigenen</p>' +
                '</header>' +
                '<div class="editable allow-backspace-key" contenteditable="true"></div>' +
                '<footer>' +
                '<div class="tools">' +
                '<span title="Vorschau-Text zurücksetzen" class="reset-preview"></span>' +
                '<span title="Verringere die Vorschaugröße" class="size-down"></span>' +
                '<span title="Erhöhe die Vorschaugröße" class="size-up"></span>' +
                '<span title="Verwende diese Schriftart" class="use-font"></span>' +
                '</div>' +
                '</footer>' +
                '</div>');
            ...
            /* preview functions */
            this.defaultPreviewText = 'Der schnelle Braunfuchs springt über den faulen Hund.';
    ...
    fontBrowserOpen = function(event) {
        ...
        if (fontBrowser.data('visible') !== true) {
            ...
            UpFront.iframe.contents().bind('mousedown', { fontBrowser: fontBrowser }, fontBrowserClose);
            ...
            UpFront.iframe.contents().off('mousedown', fontBrowserClose);
