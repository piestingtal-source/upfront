    ...
    bindGridManager = function() {
        ...
        $('div#boxes').delegate('span#grid-manager-button-preset-use-preset', 'click', function() {
            ...
            $.each(gridWizardPresets[selectedPreset], function() {
                ...
                $i('.ui-upfront-grid').first().data('ui-upfrontGrid').addBlock(addBlockArgs);
        ...
        $('div#boxes').delegate('span#grid-manager-button-clone-page', 'click', function() {
            ...
            $(this).text('Klonen...').addClass('button-depressed').css('cursor', 'default');

            var request = $.ajax(UpFront.ajaxURL, {
                type: 'POST',
                async: true,
                data: {
                ...
                success: function(data, textStatus) {
                    ...
                    $.each(blocks, function() {
                        ...
                        if (typeof wrapperIDTranslations[blockWrapper.replace('wrapper-', '')] != 'undefined') {
                            ...
                            var destinationWrapper = $i('.ui-upfront-grid').filter(destinationWrapperID).first();

                        } else {

                            var destinationWrapper = $i('.ui-upfront-grid').last();

                        }

                        /* Add block to wrapper */
                        var newBlock = destinationWrapper.data('ui-upfrontGrid').addBlock(addBlockArgs);
        ...
        $('div#boxes').delegate('span#grid-manager-button-assign-template', 'click', function() {
            ...
            if (templateToAssign === '')
                return alert('Bitte wähle ein freigegebenes Layout aus, das zugewiesen werden soll.');
            ...
            $.post(UpFront.ajaxURL, {
                ...
                UpFront.viewModels.layoutSelector.currentLayoutTemplate('template-' + templateToAssign);
                UpFront.viewModels.layoutSelector.currentLayoutTemplateName($('span.layout[data-layout-id="template-' + templateToAssign + '"]').find('.template-name').text());

                //Reload iframe and new layout
                upfrontIframeLoadNotification = 'Geteiltes Layout erfolgreich zugewiesen!';
        ...
        initiateLayoutImport = function(input) {
            ...
            if (!layoutChooser.val())
                return alert('Du musst vor dem Import eine UpFront-Layoutdatei auswählen.');
            ...
            if (layoutFile && typeof layoutFile.name != 'undefined' && typeof layoutFile.type != 'undefined') {
                ...
                layoutReader.onload = function(e) {
                    ...
                    if (layout['data-type'] != 'layout')
                        return alert('Layoutdatei kann nicht geladen werden. Stelle sicher, dass es sich bei der ausgewählten Datei um einen gültigen UpFront-Layout-Export handelt.');
                    ...
                    if (typeof layout['image-definitions'] != 'undefined' && Object.keys(layout['image-definitions']).length) {
                        ...
                        $.post(UpFront.ajaxURL, {
                            ...
                            if (typeof layout['error'] != 'undefined')
                                return alert('Fehler beim Importieren von Bildern für das Layout: ' + layout['error']);
                ...
                alert('Layoutdatei kann nicht geladen werden. Stelle sicher, dass es sich bei der ausgewählten Datei um einen gültigen UpFront-Layout-Export handelt.');
        ...
        importLayout = function(layout) {
            ...
            showNotification({
                ...
                message: 'Layout erfolgreich importiert.<br /><br />Denke daran, zu speichern, wenn Du das Layout beibehalten  möchtest.',
                closeTimer: false,
                closable: true,
                success: true
            });
        ...
        $('div#boxes').delegate('#grid-manager-import input[type="file"]', 'change', function(event) {
            ...
            if (event.target.files[0].name.split('.').slice(-1)[0] != 'json') {
                ...
                $(this).val(null);
                return alert('Ungültige Layoutdatei. Stelle sicher, dass das Layout eine gültige JSON-formatierte Datei ist.');
