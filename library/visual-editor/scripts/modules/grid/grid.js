define(['jquery', 'helper.history', 'helper.data'], function($, history) {
    ...
    $.widget("ui.upfrontGrid", $.ui.mouse, {

        options: {
            useIndependentGrid: false,
            columns: UpFront.defaultGridColumnCount,
            columnWidth: UpFront.globalGridColumnWidth,
            gutterWidth: UpFront.globalGridGutterWidth,
            yGridInterval: 5,
            minBlockHeight: 10,
        ...
        _create: function() {
            ...
            this.options.columnWidth = this.wrapper.data('wrapper-settings')['column-width'] ? this.wrapper.data('wrapper-settings')['column-width'] : UpFront.globalGridColumnWidth;
            this.options.gutterWidth = this.wrapper.data('wrapper-settings')['gutter-width'] ? this.wrapper.data('wrapper-settings')['gutter-width'] : UpFront.globalGridGutterWidth;
            ...
            this.wrapper.addClass('ui-upfront-grid');
            ...
            this.container.delegate('.' + this.options.defaultBlockClass.replace('.', ''), 'dblclick', function(event) {
                ...
                var grid = $(this).parents('.ui-upfront-grid').data('ui-upfrontGrid');
        ...
        iframeMouseMove: function(event) {
            ...
            if (typeof this._iframeMouseDownEvent != 'undefined' || typeof this._doingHoverBlockToTop != 'undefined' || !UpFront.touch)
                return;
        ...
        _mouseDrag: function(event) {
            ...
            if (blockLeft + blockWidth > (this.grid.columns * (this.grid.columnWidth + this.grid.gutterWidth)))
                UpFront.blankBlockOptions.width = containerWidth - UpFront.blankBlockOptions.left;

            /* If block bleeds out bottom, put a damper there. */
            if (event.pageY > (containerTop + containerHeight)) {
                UpFront.blankBlockOptions.height = containerHeight - blockTop;
            }

            /* End Maxes */

            /* Apply the CSS */
            this.helper.css(UpFront.blankBlockOptions);

            /* Change width/left to classes to keep finicky bullshit to a minimum */
            var widthGridNum = Math.round((UpFront.blankBlockOptions.width + this.grid.gutterWidth) / (this.grid.columnWidth + this.grid.gutterWidth));
            var leftGridNum = Math.round(UpFront.blankBlockOptions.left / (this.grid.columnWidth + this.grid.gutterWidth));
            ...
            if (UpFront.blankBlockOptions.height < this.options.minBlockHeight) {
        ...
        _mouseStop: function(event) {
            ...
            if (UpFront.blankBlockOptions.pixelWidth < this.grid.columnWidth || UpFront.blankBlockOptions.height < this.options.minBlockHeight)
                return false;

            this.addBlankBlock(UpFront.blankBlockOptions);
        ...
        resizableStart: function(event, ui) {
            ...
            var grid = block.parents('.ui-upfront-grid').data('ui-upfrontGrid');
        ...
        draggableStart: function(event, ui) {
            ...
            var block = ui.el;
            var grid = getBlock(block).parents('.ui-upfront-grid').data('ui-upfrontGrid');
        ...
        draggableStop: function(event, ui) {
            ...
            if (isOverlapping && $i('.grid-container').length > 1) {
                ...
                var destinationContainerUpFrontGrid = destinationContainer.parents('.wrapper').data('ui-upfrontGrid');
                ...
                destinationContainer.parents('.wrapper').upfrontGrid('initResizable', block);

                $.pep.off(block);
                destinationContainer.parents('.wrapper').upfrontGrid('initDraggable', block);
                ...
                block.parents('.ui-upfront-grid').first().data('ui-upfrontGrid').alignBlockWithGuides(block);
            ...
            if (blocks.length === 1) {
                var description = 'Block verschoben';
            } else {
                var description = 'Massenverschobene Blöcke: ';
            }
        ...
        addBlankBlock: function(args, usingAddBlock) {
            ...
            UpFront.blankBlock = $('<div><div class="block-content-fade block-content"></div></div>')
            ...
            updateBlockContentCover(UpFront.blankBlock);

            var block = UpFront.blankBlock;
            ...
            if (usingAddBlock == false) {
                openBlockTypeSelector($(UpFront.blankBlock));
            }
        ...
        setupBlankBlock: function(blockType, usingAddBlock, loadContent) {
            ...
            UpFront.blankBlock.removeClass('blank-block');
            UpFront.blankBlock.addClass('block-type-' + blockType);
            UpFront.blankBlock.data('type', blockType);

            if (loadContent) {

                loadBlockContent({
                    blockElement: UpFront.blankBlock,
                    blockOrigin: {
                        type: blockType,
                        id: 0,
                    ...
                    blockSettings: {
                        dimensions: getBlockDimensions(UpFront.blankBlock),
                        position: getBlockPosition(UpFront.blankBlock)
                    },
                });
            ...
            if (getBlockTypeObject(blockType)['fixed-height'] === true) {
                ...
                UpFront.blankBlock.addClass('block-fixed-height');

            } else {

                UpFront.blankBlock.addClass('block-fluid-height');

            }

            //Set the hide-content-in-grid depending on the block type
            if (getBlockTypeObject(blockType)['show-content-in-grid'])
                UpFront.blankBlock.removeClass('hide-content-in-grid');

            //Add the hidden input flag
            dataAddBlock(UpFront.blankBlock);
            dataSetBlockPosition(getBlockID(UpFront.blankBlock), getBlockPosition(UpFront.blankBlock));
            dataSetBlockDimensions(getBlockID(UpFront.blankBlock), getBlockDimensions(UpFront.blankBlock));
            dataSetBlockWrapper(getBlockID(UpFront.blankBlock), UpFront.blankBlock.closest('.wrapper').attr('id'));

            //Check for intersectors and allow saving if possible
            if (blockIntersectCheck(UpFront.blankBlock)) {
                allowSaving();
            } else {
                disallowSaving();
            }
            ...
            //Clear variable
            delete UpFront.blankBlock;
            delete UpFront.blankBlockOptions;
        ...
        updateGridCSS: function() {
            ...
            if (
                ...
                updateGridCSS(wrapperSelector, this.options.columns, UpFront.globalGridColumnWidth, UpFront.globalGridGutterWidth, gridWidthInputContext);
            }
