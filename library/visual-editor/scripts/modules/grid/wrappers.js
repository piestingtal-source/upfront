define(['util.custommouse', 'qtip', 'helper.data', 'modules/grid/wrapper-inputs'], function() {
    ...
    setupWrapperSortables = function() {
        ...
        $i('#whitewrap div.wrapper-drag-handle, #whitewrap div.wrapper-options').qtip({
            ...
            position: {
                my: 'right center',
                at: 'left center',
                container: UpFront.iframe.contents().find('body'),
        ...
        return $i('#whitewrap').sortable({
            ...
            start: function(event, ui) {
                ...
                UpFront.iframe.data('maximumScrollTop', UpFront.iframe.contents().height());
                ...
                if (event.clientY < 100) {
                    ...
                    if (typeof wrapperDraggableScrollUpInterval == 'undefined') {
                        ...
                        wrapperDraggableScrollUpInterval = setInterval(function() {
                            UpFront.iframe.contents().scrollTop(UpFront.iframe.contents().scrollTop() - 8);
                        }, 5);

                    }

                    /* Automatically scroll down */
                } else if ((UpFront.iframe.height() - event.clientY) < 100) {
                    ...
                    if (typeof wrapperDraggableScrollDownInterval == 'undefined') {
                        ...
                        wrapperDraggableScrollDownInterval = setInterval(function() {
                            ...
                            var newScrollTop = UpFront.iframe.contents().scrollTop() + 8;

                            /* Do not allow scrollTop to exceed the document height */
                            if (UpFront.iframe.height() + newScrollTop >= UpFront.iframe.data('maximumScrollTop')) {
                                newScrollTop = UpFront.iframe.data('maximumScrollTop') - UpFront.iframe.height();
                            }

                            UpFront.iframe.contents().scrollTop(newScrollTop);
            ...
            stop: function(event, ui) {
                ...
                $i('.wrapper').each(function() {
                    $(this).upfrontGrid('updateGridContainerHeight');
                });
            ...
            contentsCallback: function(event) {
                ...
                if (wrapper.hasClass('wrapper-fluid') && UpFront.mode == 'grid') {
                    ...
                    $('<li class="context-menu-wrapper-to-fixed"><span>Fester Container</span></li>').appendTo(contextMenu).find('span').on('click', function() {
                        ...
                        wrapper.data('ui-upfrontGrid').resetGridCalculations();
                        wrapper.data('ui-upfrontGrid').alignAllBlocksWithGuides();
                        wrapper.data('ui-upfrontGrid').updateGridContainerHeight();

                    });

                    if (wrapper.hasClass('wrapper-fixed-grid')) {

                        $('<li class="context-menu-wrapper-grid-to-fluid"><span>Ändere Gitter zu Flüssig</span></li>').appendTo(contextMenu).find('span').on('click', function() {
                        ...
                        $('<li class="context-menu-wrapper-grid-to-fixed"><span>Ändere Gitter zu Fest</span></li>').appendTo(contextMenu).find('span').on('click', function() {
    ...
    addWrapper = function(position, wrapperSettings, suppressNotice) {
        ...
        /* Initiate UpFront Grid on new wrapper */
