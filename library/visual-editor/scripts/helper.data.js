define(['underscore'], function(_) {
	...
	dataAddWrapper = function(wrapper, settings, position) {
		...
		GLOBALunsavedValues['wrappers'][wrapperID] = {
			...
			settings: jQuery.extend({}, {
				...
				'column-width': UpFront.globalGridColumnWidth,
				'gutter-width': UpFront.globalGridGutterWidth
			}, settings)
		};
	...
	dataHandleDesignEditorInput = function(args) {
		...
		$('#design-editor-element-selector-container').find('li#element-' + element)
			...
			.attr('title', 'Du hast eine Eigenschaft in dieser Eigenschaftsgruppe angepasst.');
	...
	dataDesignEditorPropertyFeedback = function(args) {
		...
		if ( !specialElementType ) {
			...
			selector = UpFront.elements[element]['selector'];

		} else if ( specialElementType == 'layout' ) {

			var originalSelector = UpFront.elements[element]['selector'].replace('body', '');
			...
			selector = UpFront.elements[element][specialElementType + 's'][specialElementMeta]['selector'];

		}

		/* Call developer-defined callback */
		if ( UpFront.designEditorProperties.hasOwnProperty(property) ) {

			var callback = eval('(function(params){' + UpFront.designEditorProperties[property]['js-callback'] + '})');
	...
	dataSetDesignEditorProperty = function(args) {
		...
		if ( specialElementType == false || specialElementMeta == false ) {
			...
			/* Change UpFront.elementData as well that way other places can update before saving */
			if ( typeof UpFront.elementData != 'object' )
				UpFront.elementData = new Object();

			if ( typeof UpFront.elementData[element] == 'undefined' )
				UpFront.elementData[element] = {properties: {}};

			if ( typeof UpFront.elementData[element]['properties'] == 'undefined' )
				UpFront.elementData[element]['properties'] = {};

			UpFront.elementData[element]['properties'][property] = value;
			...
			if ( typeof UpFront.elementData[element] != 'object' )
				UpFront.elementData[element] = new Object();

			if ( typeof UpFront.elementData[element]['special-element-' + specialElementType] != 'object' )
				UpFront.elementData[element]['special-element-' + specialElementType] = new Object();

			if ( !_.isObject(UpFront.elementData[element]['special-element-' + specialElementType][specialElementMeta]) || _.isArray(UpFront.elementData[element]['special-element-' + specialElementType][specialElementMeta]) )
				UpFront.elementData[element]['special-element-' + specialElementType][specialElementMeta] = new Object();

			UpFront.elementData[element]['special-element-' + specialElementType][specialElementMeta][property] = value;
