define(['jquery', 'underscore'], function($, _) {
    ...
    openImageUploader = function(callback) {
        ...
        if (!boxExists('input-image')) {
            ...
            if (isNaN(UpFront.viewModels.layoutSelector.currentLayout()))
                iframePostID = 0;

            var settings = {
                id: 'input-image',
                title: 'Wähle ein Bild aus',
                description: 'Lade ein Bild hoch oder wähle es aus',
                src: UpFront.homeURL + '/?upfront-trigger=media-uploader',
                load: function() {
    ...
    initiateImageUploader = function(callback) {
        ...
        if (!$('#box-input-image iframe').length ||
        ...
        var currentLayoutFragments = UpFront.viewModels.layoutSelector.currentLayout().split('-');
