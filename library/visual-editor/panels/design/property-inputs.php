...
class UpFrontPropertyInputs {
	...
	public static function display($element, $special_element_type = false, $special_element_meta = false, $data, $data_without_defaults) {
		...
		/* Include nudging and margin for all elements except for Body */
			if ( !(!empty($element['disallow-nudging']) && upfront_fix_data_type($element['disallow-nudging']) === true) && !in_array('nudging', $element['properties']) ) {
	...
	public static function box($args) {
		...
		if ( !($properties = UpFrontElementProperties::get_properties_by_group($args['group_nice'])) )
			return false;
		...
		/* Custom behaviors for special element types */
			switch ( $args['special_element_type'] ) {
				...
				case 'instance':
					...
					$instances = upfront_get('instances', $args['element']);
				...
				case 'state':
					...
					$states = upfront_get('states', $args['element']);
		...
		/* Set customized box class flag */
			...
			foreach ( $args['property_values_excluding_defaults'] as $property_id => $property_value ) {
				...
				$property_box_title = ' title="' . __('Du hast eine Eigenschaft in dieser Eigenschaftsgruppe angepasst.', 'upfront') . '"';
			...
			echo '<div class="design-editor-box design-editor-box-' . $args['group'] . $customized_box_class . '">';
				...
				echo '<ul class="design-editor-box-content">';
					...
					foreach ( $properties as $property_id => $property_options ) {
						...
						if ( !upfront_get('display', $property_options, true) )
							continue;
						...
						/* Handle box model inputs differently from the rest of the property inputs */							
							echo '<div class="box-model-inputs-container">';
								...
								if ( upfront_get('name', $property_options) )
									echo '<strong class="box-model-inputs-heading">' . $property_options['name'] . '</strong>';

								echo '<div class="box-model-inputs box-model-inputs-position-' . upfront_get('position', $property_options, 'sides') . '">';
		...
		public static function build_property_input($property_id, $property_options, $element_args) {
			...
			/* Get the value of the property */
				$original_property_value = upfront_fix_data_type(upfront_get($property_id, $element_args['property_values']));
			...
			/* Set up elements and attributes */
				...
				$customize_button = $element_args['special_element_type'] != 'default' ? '<div class="customize-property"><span class="tooltip" title="Klicke hier, um den Wert für diese Eigenschaft zu ändern. Wenn die Eigenschaft nicht angepasst wird, erbt sie automatisch den Standardsatz für diesen Elementtyp auf der Registerkarte "Standard" oder das übergeordnete Element, wenn ein Status, eine Instanz oder ein layoutspezifisches Element bearbeitet wird.">Anpassen</span></div>' : null;
			...
			/* Set up attributes */
				...
				if ( $property_options['customized'] ) {
					...
					$property_title = ' title="' . __('You have customized this property.', 'upfront') . '"';
			...
			/* add a locked class if it's a lockable element only */
				if ( upfront_get('lockable', $property_options) )
					$property_classes[] = 'lockable-property';

			/* set the customizable defaults and class */
			if ( is_array( upfront_get( 'unit', $property_options ) ) ) {
			...
			echo '<li data-property-id="' . $property_id . '" class="' . implode(' ', array_filter($property_classes)) . '"' . $property_title . '>';
				...
				echo '<strong><span class="property-label">' . $property_options['name'] . '</span>' . (!upfront_get('lockable', $property_options) ? $uncustomize_button : null) . '</strong>';
				echo '<div class="property-' . $property_options['type'] . ' property">';

					echo (upfront_get('lockable', $property_options)) ? $uncustomize_button : null; /* Uncustomize button needs to be in different location for box model input s*/
	...
	public static function input_integer($options, $id) {
		...
		$unit = is_string(upfront_get('unit', $options)) ? '<span class="unit">' . upfront_get('unit', $options) . '</span>' : null;

		/* Remove unit from value */
		if ( is_array( upfront_get( 'unit', $options ) ) ) {
		...
		echo '<input type="number" value="' . $options['value'] . '" step="' . upfront_get('step', $options, 1) . '"  />' . $unit;
	...
	public static function input_color($options, $id) {
		...
		<div class="colorpicker-box-container">
			...
			<div class="colorpicker-box" style="background-color:' . upfront_format_color($options['value']) . ';"></div>
	...
	public static function input_select($options, $id = null) {
		...
		$unit_select_class = upfront_get('unit-select', $options) ? ' property-unit-select' : '';
	...
	public static function input_font_family_select($options, $id) {
		...
		/* Output input */
			...
			if ( count($font_fragments) >= 2 ){
				...
				/* Traditional Font */
				$font_stack = UpFrontFonts::get_stack($options['value']);
		...
		/* Font Browser */
			echo '<div class="font-browser">';
					...
					echo '<ul class="tabs">';
						do_action('upfront_fonts_browser_tabs');
					echo '</ul>';

					do_action('upfront_fonts_browser_content');
