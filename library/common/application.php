<?php
/**
 * All of the global functions to be used everywhere in UpFront.
 *
 * @package UpFront
...
class UpFront {
	...
	/**
	 * Let's get UpFront on the road!  We'll define constants here, run the setup function and do a few other fun things.
	 *
	 * @return void
	 **/
	public static function init() {
		...
		$GLOBALS['upfront_default_element_data'] = array();

		/* Define simple constants */
		if ( ! defined( 'THEME_FRAMEWORK' ) ) {
			define( 'THEME_FRAMEWORK', 'upfront' );
		}
		if ( ! defined( 'UPFRONT_VERSION' ) ) {
			define( 'UPFRONT_VERSION', '1.3.12' );
		}

		/* Define directories */
		if ( ! defined( 'UPFRONT_DIR' ) ) {
			define( 'UPFRONT_DIR', get_template_directory() );
		}
		if ( ! defined( 'UPFRONT_LIBRARY_DIR' ) ) {
			define( 'UPFRONT_LIBRARY_DIR', upfront_change_to_unix_path( UPFRONT_DIR . '/library' ) );
		}

		/* Dev Params */
		if ( file_exists( UPFRONT_LIBRARY_DIR . '/dev-env.php' ) ) {
			require_once UPFRONT_LIBRARY_DIR . '/dev-env.php';
		}

		UpFrontSettings::set_enviroment();
		...
		/* Skins */
		if ( ! defined( 'UPFRONT_DEFAULT_SKIN' ) ) {
			define( 'UPFRONT_DEFAULT_SKIN', 'base' );
		}
		 ...
		 * Parse PHP
		 *
		 * English: https://www.facebook.com/groups/upfrontunlimitedEN/permalink/641292209707584/
		 * Español: https://www.facebook.com/groups/upfrontunlimitedES/permalink/383744532348603/
		 */
		if ( ! defined( 'UPFRONT_DISABLE_PHP_PARSING' ) ) {
			define( 'UPFRONT_DISABLE_PHP_PARSING', false );
		}
		...
		if ( get_template_directory_uri() !== get_stylesheet_directory_uri() ) {
			...
			if ( ! defined( 'UPFRONT_CHILD_THEME_ACTIVE' ) ) {
				define( 'UPFRONT_CHILD_THEME_ACTIVE', true );
			}
			if ( ! defined( 'UPFRONT_CHILD_THEME_DIR' ) ) {
				define( 'UPFRONT_CHILD_THEME_DIR', get_stylesheet_directory() );
			}
		} else {
			if ( ! defined( 'UPFRONT_CHILD_THEME_ACTIVE' ) ) {
				define( 'UPFRONT_CHILD_THEME_ACTIVE', false );
			}
			if ( ! defined( 'UPFRONT_CHILD_THEME_DIR' ) ) {
				define( 'UPFRONT_CHILD_THEME_DIR', null );
			}
		}
		...
		if ( ! defined( 'UPFRONT_UPLOADS_DIR' ) ) {
			define( 'UPFRONT_UPLOADS_DIR', upfront_change_to_unix_path( $uploads['basedir'] . '/upfront' ) );
		}
		if ( ! defined( 'UPFRONT_CACHE_DIR' ) ) {
			define( 'UPFRONT_CACHE_DIR', upfront_change_to_unix_path( UPFRONT_UPLOADS_DIR . '/cache' ) );
		}

		/* Make directories if they don't exist */
		if ( ! is_dir( UPFRONT_UPLOADS_DIR ) ) {
			wp_mkdir_p( UPFRONT_UPLOADS_DIR );
		}

		if ( ! is_dir( UPFRONT_CACHE_DIR ) ) {
			wp_mkdir_p( UPFRONT_CACHE_DIR );
		}

		self::add_index_files_to_uploads();

		/* Load locale */
		load_theme_textdomain( 'upfront', upfront_change_to_unix_path( UPFRONT_LIBRARY_DIR . '/languages' ) );
	...
	public static function activate() {
		...
		add_option( 'upfront-disable-automatic-core-updates', '0', '', 'no' );
		add_option( 'upfront-disable-automatic-plugin-updates', '0', '', 'no' );
	...
	public static function deactivate() {
		...
		delete_option( 'upfront-disable-automatic-core-updates' );
		delete_option( 'upfront-disable-automatic-plugin-updates' );
	...
	public static function add_index_files_to_uploads() {
		...
		$uploads_index = trailingslashit( UPFRONT_UPLOADS_DIR ) . 'index.php';
		$cache_index = trailingslashit( UPFRONT_CACHE_DIR ) . 'index.php';
	...
	public static function load_dependencies() {
		...
		if ( UpFrontRoute::is_visual_editor() || ( defined( 'DOING_AJAX' ) && DOING_AJAX && strpos( $_REQUEST['action'], 'upfront' ) !== false ) ) {
		...
		// Load stuff now.
		self::load( apply_filters( 'upfront_dependencies', $dependencies ) );
		do_action( 'upfront_setup' );

	}


	/**
	 * Tell WordPress that UpFront supports its features.
	 **/
	public static function add_theme_support() {

		/* UpFront Functionality */
		add_theme_support( 'upfront-grid' );
		add_theme_support( 'upfront-responsive-grid' );
		add_theme_support( 'upfront-design-editor' );

		/* UpFront CSS */
		add_theme_support( 'upfront-reset-css' );
		add_theme_support( 'upfront-live-css' );
		add_theme_support( 'upfront-block-basics-css' );
		add_theme_support( 'upfront-dynamic-block-css' );
		add_theme_support( 'upfront-content-styling-css' );
		add_theme_support( 'upfront-animation-css' );
		...
		require_once UPFRONT_LIBRARY_DIR . '/resources/dynamic-loop.php';
	...
	public static function child_theme_setup() {
		...
		if ( ! UPFRONT_CHILD_THEME_ACTIVE ) {
			return false;
		}

		do_action( 'upfront_setup_child_theme' );
	...
	public static function maybe_db_upgrade() {
		...
		if ( 0 === $db_version && ! get_option( 'upfront_option_group_general' ) ) {
			...
			$upfront_settings            = get_option( 'upfront', array( 'version' => 0 ) );
			$upfront_settings['version'] = UPFRONT_VERSION;

			update_option( 'upfront', $upfront_settings );

			return $upfront_settings;

		}

		/* If the version in the database is already up to date, then there are no upgrade functions to be ran. */
		if ( version_compare( $db_version, UPFRONT_VERSION, '>=' ) ) {
			if ( get_option( 'upfront_upgrading' ) ) {
		...
		return UpFrontMaintenance::do_upgrades();
	...
	public static function set_autoload( $template = null ) {
		...
		$wpdb->query( "UPDATE $wpdb->options SET autoload = 'no' WHERE option_name LIKE 'upfront_%'" );
		...
		$wpdb->update(
			...
			array(
				'option_name' => 'upfront_option_group_general',
			)
		);
	...
	/**
	 * Initiate the UpFront Theme updater checker class for UpFront itself.
	 **/
	public static function initiate_updater() {
		UpFrontCoreUpdater::updater();
	}
	...
	public static function load( $classes, $init = false ) {
		...
		foreach ( $load as $file => $init ) {
			...
			if ( true === $init ) {
				...
				if ( strpos( $file, '/' ) !== false ) {
					require_once UPFRONT_LIBRARY_DIR . '/' . $file . '.php';
				}
			}
		}
		...
		foreach ( $classes_to_init as $class ) {
			...
			if ( method_exists( 'UpFront' . $class, 'init' ) ) {

				call_user_func( array( 'UpFront' . $class, 'init' ) );

			} else {

				trigger_error( 'UpFront' . $class . '::init is not a valid method', E_USER_WARNING );
	...
	public static function get() {
		_deprecated_function( __FUNCTION__, '1.0.0', 'upfront_get()' );
		$args = func_get_args();
		return call_user_func_array( 'upfront_get', $args );
	}
	...
	public static function post() {
		_deprecated_function( __FUNCTION__, '1.0.0', 'upfront_post()' );
		$args = func_get_args();
		return call_user_func_array( 'upfront_post', $args );
	}
