...
class UpFrontBlocksAnywhere {

	private static $blocks = array();

	static function init() {

		add_filter( 'upfront_compiler_trigger_url', array( __CLASS__, 'add_current_layout' ) );
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'enqueue_assets' ) );
		add_shortcode( 'upfront-block', array( __CLASS__, 'create_block_shortcode' ) );

	}


	/* we add the current layout to the compiler uncached files so that we can call it using upfront_get('current-layout') later on */
	static function add_current_layout( $url ) {

		return add_query_arg( array( 'current-layout' => UpFrontLayout::get_current() ), $url );
	...
	static function create_block_shortcode( $atts = [], $content = null, $tag = '' ) {
		...
		$current_layout = UpFrontLayout::get_current();
		...
		$layout_id = !empty( $post_id ) ? $post_id : upfront_get_int( $block['layout'] );
		...
		if ( empty( $layout_id ) && $block['type'] == 'content' && $mode == 'default' ) {
			...
			echo '<div class="alert alert-red">' . __('Diesem Block ist kein Beitrag zugeordnet. Bitte gib die post_id an, die Du als Shortcode-Parameter anzeigen möchtest.', 'upfront') . '</div>';
			...
			UpFrontBlocks::display_block( $block['id'] );
		...
		if ( strlen( $output_string ) == 0 )
			return '<div class="alert alert-yellow">' . __('Die von Dir eingegebene UpFront-Blockverknüpfung ist nicht vorhanden. Bitte gib eine gültige Block ID ein!', 'upfront') . '</div>';
		else
	...
	static function enqueue_assets() {
		...
		/* we prevent this from firing in the admin and VE. we added the upfront call condition for certain plugins template pages */
		...
		if ( self::dynamic_js() != '' ) {
			...
			$script_name = 'upfront-blocks-anywhere-dynamic-js-layout-' . UpFrontLayout::get_current();

			UpFrontCompiler::register_file( array(
				'name' => $script_name,
				'format' => 'js',
				'fragments' => array(
					array( 'UpFrontBlocksAnywhere', 'dynamic_js' )
				),
				'enqueue' => false
			) );
		...
		if ( self::dynamic_css() != '' ) {
			...
			$css_name = 'upfront-blocks-anywhere-css-layout-' . UpFrontLayout::get_current();

			$fragments['dynamic-block-css'] = array('UpFrontBlocksAnywhere', 'dynamic_css');
	...
	static function get_assets( $type ) {
		...
		if ( !$current_layout )
			$current_layout = upfront_get( 'current-layout' );

		if ( version_compare('0.0.17', UPFRONT_VERSION, '<=') )
		...
		if ( isset( $post->post_content ) ) {
			...
			preg_match_all( '/\[upfront\-block id=["|\'](.*?)["|\']/', $post->post_content, $this_shortcodes_ids );
		...
		if ( !empty( $shortcodes_ids ) ) {
			...
			foreach ( $shortcodes_ids as $block_id ) {
				...
				$block_options = upfront_get( $block['id'], UpFrontBlocks::$block_actions['block-objects'] );
				$original_block = null;
				...
				if ( $possible_mirror_id = UpFrontBlocksData::is_block_mirrored( $block_options, true ) ) {
	...
	static function get_block( $block_id ) {
		...
		$block_options = upfront_get( $block_id, UpFrontBlocks::$block_actions['block-objects'] );
		$original_block = null;
