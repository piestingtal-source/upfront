...
class UpFrontLayout {
	...
	public static function init() {
		...
		add_action('upfront_flush_cache', array(__CLASS__, 'clear_status_transient'));
	...
	public static function clear_status_transient() {
		...
		delete_transient('pu_customized_layouts_template_' . UpFrontOption::$current_skin);
		delete_transient('pu_layouts_with_templates_template_' . UpFrontOption::$current_skin);
	...
	/**
	 * Check if layout exists via UpFrontLayout::get_name()
	 **/
	...
	public static function get_current() {
		...
		if ( upfront_get('ve-layout') && (UpFrontRoute::is_visual_editor_iframe() || UpFrontRoute::is_visual_editor()) ) 
			return urldecode(upfront_get('ve-layout'));

		$current_hierarchy = self::get_current_hierarchy();

		return apply_filters('upfront_current_layout', end($current_hierarchy));
	...
	public static function get_current_in_use($visual_editor_force = false) {
		...
		//If the user is viewing the site through the iframe and the mode is set to Layout, then display that exact layout.
			if ( upfront_get('ve-layout') && !$visual_editor_force ) {

                if ( UpFrontRoute::is_visual_editor_iframe('grid') || (UpFrontRoute::is_visual_editor() && upfront_get('visual-editor-mode') == 'grid') ) {
                    return upfront_get('ve-layout');
                }

                if ( UpFrontRoute::is_visual_editor_iframe('design') ) {

                    if ( strpos(upfront_get('ve-layout'), 'template-') === 0 || upfront_get('ve-layout-customized') == 'true' ) {
		...
		foreach ( $top_level_layouts as $top_level_layout ) {
			...
			if ( $status['customized'] && count(UpFrontBlocksData::get_blocks_by_layout($top_level_layout)) > 0 )
				return $top_level_layout;

		}

		//If there STILL isn't a customized layout, just return the top level of the current layout.
		return apply_filters('upfront_current_layout_in_use', end($hierarchy));
	...
	public static function get_current_hierarchy() {
		...
		if ( !empty($GLOBALS['upfront_current_hierarchy']) ) {
			return apply_filters('upfront_current_layout_hierarchy', $GLOBALS['upfront_current_hierarchy']);
		}

        if ( UpFrontRoute::is_visual_editor() && upfront_get( 've-layout' ) ) {
            return explode(UpFrontLayout::$sep, urldecode(upfront_get('ve-layout')));
        }
		...
		if ( count($current_layout) ) {
			$GLOBALS['upfront_current_hierarchy'] = $current_layout;
		}

		return apply_filters('upfront_current_layout_hierarchy', $current_layout);
	...
	public static function get_layout_parents_names($layout) {
		...
		$layout_id_fragments = explode( UpFrontLayout::$sep, $layout );
	...
	public static function get_status($layout, $include_post_status = false) {
		...
		/* Get the customized transient */
			$transient_id_customized_layouts = 'pu_customized_layouts_template_' . UpFrontOption::$current_skin;
		...
		/* Get the templates status transient */
			$transient_id_layouts_with_templates = 'pu_layouts_with_templates_template_' . UpFrontOption::$current_skin;
		...
		/* Get the template */
			$possible_template = ( $layout_parts[0] == 'single' && is_numeric($layout_end_part) ) ? upfront_get($layout_end_part, $layouts_with_templates) : upfront_get($layout, $layouts_with_templates);
	...
	public static function set_layout_status_customized_transient() {
		...
		$customized_layouts = array_unique( $wpdb->get_col( $wpdb->prepare( "SELECT layout FROM $wpdb->pu_blocks WHERE template = '%s'", UpFrontOption::$current_skin ) ) );
	...
	public static function set_layout_status_templates_transient() {
		...
		$templated_layouts_pu_meta = $wpdb->get_results( $wpdb->prepare( "SELECT layout, meta_value FROM $wpdb->pu_layout_meta WHERE meta_key = '%s' AND meta_value <> '' AND template = '%s'", 'template', UpFrontOption::$current_skin ) );
		$templated_layouts_wp_meta = $wpdb->get_results( $wpdb->prepare( "SELECT post_id, meta_value FROM $wpdb->postmeta WHERE meta_key = '%s' AND meta_value <> ''", '_pu_|template=' . UpFrontOption::$current_skin . '|_template' ) );
	...
	/** 
	 * Simple function to query for all UpFront layout templates from the database.
	 * 
	 * @return array
	 **/
	...
	public static function add_template($template_name = null) {
		...
		$templates = UpFrontSkinOption::get('list', 'templates', array());
		$last_template_id = UpFrontSkinOption::get('last-id', 'templates', 0);
		...
		/* Send array to DB */
			...
			UpFrontSkinOption::set('last-id', $id, 'templates');
	...
	public static function delete_layout($layout_id, $skin_only = true) {
		...
		UpFrontBlocksData::delete_by_layout($layout_id);

		//Delete wrappers and wrapper instances
		UpFrontWrappersData::delete_by_layout($layout_id);

		//Delete the layout options for this layout
		if ( $skin_only ) {
			UpFrontLayoutOption::delete_all_from_layout($layout_id);
		} else {
			UpFrontLayoutOption::delete_all_from_layout($layout_id, true);
		}
