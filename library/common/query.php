...
class UpFrontQuery{
	 ...
	 * Query posts
	 *
	 * @param Block $block Block to work with.
	 * @return array
	 */
	public static function get_posts( $block ) {

		$categories      = UpFrontBlockAPI::get_setting( $block, 'categories', array() );
		$categories_mode = UpFrontBlockAPI::get_setting( $block, 'categories-mode', array() );
		$enable_tags     = UpFrontBlockAPI::get_setting( $block, 'enable-tags', array() );
		$tags            = UpFrontBlockAPI::get_setting( $block, 'tags', array() );
		$post_type       = UpFrontBlockAPI::get_setting( $block, 'post-type', array() );
		$post_status     = UpFrontBlockAPI::get_setting( $block, 'post-status', array() );
		$author          = UpFrontBlockAPI::get_setting( $block, 'author', array() );
		$number_of_posts = UpFrontBlockAPI::get_setting( $block, 'number-of-posts', array() );
		$offset          = UpFrontBlockAPI::get_setting( $block, 'offset', array() );
		$order_by        = UpFrontBlockAPI::get_setting( $block, 'order-by', array() );
		$order           = UpFrontBlockAPI::get_setting( $block, 'order', array() );
		$byid_include    = UpFrontBlockAPI::get_setting( $block, 'byid-include', array() );
		$byid_exclude    = UpFrontBlockAPI::get_setting( $block, 'byid-exclude', array() );
	...
	public static function get_post_types() {
		...
		foreach ( $post_types as $post_type_id => $post_type ) {
			...
			if ( in_array( $post_type_id, array( 'revision', 'nav_menu_item' ), true ) ) {
				continue;
			}
	...
	public static function get_taxonomies() {
		...
		$taxonomy_options      = array( __( '&ndash; Standard: Kategorie &ndash;', 'upfront' ) );
	 ...
	 * Query Meta
	 *
	 * @param array $post_types Post type to check.
	 * @return array
	 */
