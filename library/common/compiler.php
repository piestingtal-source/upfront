...
class UpFrontCompiler {
	...
	public static function init() {
		...
		add_action('upfront_visual_editor_save', array(__CLASS__, 'signal_flush_cache'));
		add_action('upfront_visual_editor_reset_layout', array(__CLASS__, 'signal_flush_cache'));
		add_action('upfront_visual_editor_delete_template', array(__CLASS__, 'signal_flush_cache'));
		add_action('upfront_visual_editor_assign_template', array(__CLASS__, 'signal_flush_cache'));
		add_action('upfront_visual_editor_unassign_template', array(__CLASS__, 'signal_flush_cache'));

		add_action('upfront_switch_skin', array(__CLASS__, 'signal_flush_cache'));
		add_action('switch_theme', array(__CLASS__, 'signal_flush_cache'));

		add_action('upfront_db_upgrade', array(__CLASS__, 'signal_flush_cache'));
		add_action('upfront_activation', array(__CLASS__, 'signal_flush_cache'));
		add_action('upfront_global_reset', array(__CLASS__, 'signal_flush_cache'));

		add_action('upfront_snapshot_rollback', array(__CLASS__, 'signal_flush_cache'));
	 ...
	 * @param bool
	 * 
	 * @uses UpFrontCompiler::enqueue_file()
	 * 
	 * @return bool
	 **/
	public static function register_file($args) {
		...
		if ( !$cache = get_transient('pu_compiler_template_' . UpFrontOption::$current_skin) )
			$cache = array();

		if ( is_ssl() )
			$args['name'] = $args['name'] . '-https';

		$args['fragments'] 		= array_map('upfront_change_to_unix_path', $args['fragments']);
		$args['dependencies'] 	= array_map('upfront_change_to_unix_path', $args['dependencies']);

		if ( !in_array($args['format'], self::$accepted_formats) )
			wp_die('<strong>' . $args['format'] .'</strong> is not an accepted filetype for the UpFrontCompiler class.');
		...
		/* If file is not registered or fragments are not the same, add it to the DB. */
			if ( $already_cached != $args ) {
				...
				//Update cache option
				if ( !set_transient('pu_compiler_template_' . UpFrontOption::$current_skin, $cache) )
					return false;
		...
		/* Output or Enqueue script */
			if ( $args['output-inline'] && $args['format'] != 'js' ) {
				...
				return add_action('wp_print_styles', function() use ($args){
					return UpFrontCompiler::output_inline($args['name']);
				});
	...
	public static function output_inline($file) {
		...
		$cache = get_transient('pu_compiler_template_' . UpFrontOption::$current_skin);
	...
	public static function enqueue_file($file, $footer_js = true) {
		...
		if ( $cache[$file]['format'] == 'js' )
			return wp_enqueue_script('upfront-' . $file, self::get_url($file), false, false, false, $footer_js);
		elseif ( $cache[$file]['format'] == 'css' )
			return wp_enqueue_style('upfront-' . $file, self::get_url($file));
	...
	public static function get_url($file) {
		...
		if ( 
			...
			&& upfront_get('filename', $cache[$file]) /* Filename in DB must be present */
			&& file_exists(UPFRONT_CACHE_DIR . '/' . upfront_get('filename', $cache[$file]))  /* Cached file must be present */
			&& !(UpFrontRoute::is_visual_editor_iframe() && !upfront_get('iframe-cache', $cache[$file])) /* Either not be iframe or if is iframe, iframe-cache must be true */
		) {

			return apply_filters('upfront_compiler_file_url', upfront_cache_url() . '/' . upfront_get('filename', $cache[$file]));

		//Cache doesn't exist	
		} else {

			//If file doesn't exist, but we can still cache, let's cache the damn thing.
			if ( self::caching_enabled() && !(UpFrontRoute::is_visual_editor_iframe() && !upfront_get('iframe-cache', $cache[$file])) ) {
			...
			//No caching available, now we have to use fallback method.
				...
				$query_args = array(
					'upfront-trigger' => 'compiler', 
					'file' => $file,
					'layout-in-use' => UpFrontLayout::get_current_in_use(),
					'rand' => rand()
				);
				...
				if ( UpFrontRoute::is_visual_editor_iframe() && upfront_get('ve-preview') )
					$query_args['ve-preview'] = 'true';

				if ( UpFrontRoute::is_theme_preview() ) {
					$query_args['preview'] = '1';
					$query_args['template'] = 'upfront';
					$query_args['stylesheet'] = 'upfront';
					$query_args['preview_iframe'] = '1';
					$query_args['TB_iframe'] = 'true';
				}

				$query_args = apply_filters( 'upfront_compiler_trigger_args', $query_args );
				$args       = apply_filters( 'upfront_compiler_trigger_url', add_query_arg( $query_args, home_url( '/' ) ));
	...
	public static function cache_file($file) {
		...
		$_GET['layout-in-use'] = UpFrontLayout::get_current_in_use(); 
		...
		//Build file
		$file_handle = @fopen(UPFRONT_CACHE_DIR . '/' . $cache[$file]['filename'], 'w');

		if ( !@fwrite($file_handle, $content) )
			return false;

		@chmod(UPFRONT_CACHE_DIR . '/' . $cache[$file]['filename'], 0644);

		@fclose($file_handle);

		set_transient('pu_compiler_template_' . UpFrontOption::$current_skin, $cache);
	...
	public static function format_content($content, $file) {
		...
		$replace = array(
			upfront_url(),
			upfront_url() . '/library',
			upfront_url() . '/library/visual-editor',
			site_url(),
			home_url()
		);
		...
		public static function strip_whitespace($content) {
			...
			if ( defined('UPFRONT_COMPILER_STRIP_WHITESPACE') && UPFRONT_COMPILER_STRIP_WHITESPACE === false )
				return $content;
	...
	public static function caching_enabled() {
		...
		if ( defined('UPFRONT_DISABLE_CACHE') && UPFRONT_DISABLE_CACHE === true )
			return false;

		if ( defined('UPFRONT_FORCE_CACHE') && UPFRONT_FORCE_CACHE === true )
			return true;

		if ( defined('WP_DEBUG') && WP_DEBUG === true )
			return false;

		if ( UpFrontOption::get('disable-caching') )
			return false;
	...
	public static function can_cache() {
		...
		if ( !is_dir(UPFRONT_CACHE_DIR) || !is_writable(UPFRONT_CACHE_DIR) )
			return false;
	...
	public static function flush_cache() {
		...
		if ( self::can_cache() ) {
			...
			delete_transient('pu_compiler_template_' . UpFrontOption::$current_skin);
			...
			if ( $handle = opendir(UPFRONT_CACHE_DIR) ) {
		...
		do_action('upfront_flush_cache');

		delete_transient('upfront_signal_flush_cache');
	...
	public static function signal_flush_cache() {
		...
		return set_transient('upfront_signal_flush_cache', true);
	...
	public static function maybe_flush_cache() {
		...
		if ( get_transient('upfront_signal_flush_cache') ) {
	...
	public static function delete_cache_file($filename) {
		...
		if ( !$filename || !file_exists(UPFRONT_CACHE_DIR . '/' . $filename) )
			return false;

		return @unlink(UPFRONT_CACHE_DIR . '/' . $filename);
