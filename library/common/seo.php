...
class UpFrontSeo {
	...
	public static function is_disabled() {
		...
		return apply_filters('upfront_seo_disabled', self::plugin_active());
	...
	public static function output_title($title, $forced_title = null) {
		...
		$seo_templates_query = UpFrontOption::get('seo-templates', 'general', self::output_layouts_and_defaults());
		$seo_templates = upfront_get(UpFrontSEO::current_seo_layout(), $seo_templates_query, array());

		if ( UpFrontLayoutOption::get(UpFrontLayout::get_current(), 'title', null, true, 'seo') )
			$title = UpFrontLayoutOption::get(UpFrontLayout::get_current(), 'title', null, true, 'seo');

		elseif ( upfront_get('title', $seo_templates) )
			$title = upfront_get('title', $seo_templates);
		...
		return UpFrontSEO::parse_seo_variables($title);
	...
	public static function output_meta() {
		...
		//Description
		if ( $seo_description = UpFrontLayoutOption::get(UpFrontLayout::get_current(), 'description', null, true, 'seo') )
			$meta .= "\n" . '<meta name="description" content="' . self::parse_seo_variables($seo_description) . '" />';

		elseif ( $seo_description = upfront_get('description', $seo_templates) )
			$meta .= "\n" . '<meta name="description" content="' . self::parse_seo_variables($seo_description) . '" />';
	...
	public static function is_seo_checkbox_enabled($option, $layout = false) {
		...
		if ( UpFrontLayoutOption::get($layout, $option, null, true, 'seo') === null && !upfront_get($option, $seo_templates) )
			return false;

		if ( UpFrontLayoutOption::get($layout, $option, null, true, 'seo') === false )
			return false;
	...
	public static function comment_rel_nofollow($url) {
		...
		if ( UpFrontOption::get('nofollow-comment-author-url') ) 
			return $url;
	...
	public static function current_seo_layout() {
		...
		$layout_hierarchy = UpFrontLayout::get_current_hierarchy();

		if ( count($layout_hierarchy) === 1 )
			return str_replace(UpFrontLayout::$sep, '-', $layout_hierarchy[0]);
		elseif ( count($layout_hierarchy) > 1 )
			return str_replace(UpFrontLayout::$sep, '-', $layout_hierarchy[1]);
	...
	public static function get_seo_templates($layout = false) {
		...
		if ( $layout )
			return upfront_get(self::current_seo_layout(), $seo_templates_query, array());
		else
