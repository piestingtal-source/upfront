<?php
class UpFrontElementProperties {
	...
	protected static $properties = array(
		...
		/* 	Fonts */
			'font-family' => array(
				...
				'complex-property' 	=> 'UpFrontElementProperties::complex_property_font_family'
			),
			...
			'font-styling' => array(
				...
				'complex-property' => 'UpFrontElementProperties::complex_property_font_styling'
			),
			...
			'capitalization' => array(
				...
				'complex-property' => 'UpFrontElementProperties::complex_property_capitalization'
			),
		...
		/* 	Fonts/Text Shadow */
			'text-shadow-horizontal-offset' => array(
				...
				'complex-property' => 'UpFrontElementProperties::complex_property_shadow',
				'default' => 0
			),
		...
		/* 	Background */
			...
			'background-overlay' => array(
				...
				'complex-property' => 'UpFrontElementProperties::complex_property_background_overlay',
				'default' => '80000000',
			),
		...
		/*	Filter		*/
			'filter' => array(
				...
				'complex-property' 	=> 'UpFrontElementProperties::complex_property_filter'
			),
		...
		'effect' => array(
				...
				'type' 				=> 'select',
				'default' 			=> 'none',				
				'complex-options' 	=> 'UpFrontElementProperties::get_effects_list',
				'js-callback' 		=> 'propertyInputCallbackEffects(params);',
				'complex-property' 	=> 'UpFrontElementProperties::complex_property_effect_content'
			),
		*/
	...
	public static function output_css($selector, $properties = array()) {
		...
		//$transformData = array();
			...
			foreach ( $properties as $property_id => $value ) {
				...
				if ( upfront_get('complex-property', $property) && is_callable(upfront_get('complex-property', $property)) ) {
					$output .= call_user_func(upfront_get('complex-property', $property), array(
						'selector' => $selector, 
						'property_id' => $property_id, 
						'value' => $value, 
						'properties' => $properties, 
				...
				if ( upfront_get('type', $property) === 'color' )
					$value = upfront_format_color($value);

				if ( upfront_get('type', $property) === 'image' && $value != 'none' )
					$value = 'url(' . $value . ')';


				// Transform support
				if ( upfront_get('group', $property) === 'Transform' ){
	...
	public static function complex_property_font_family($args) {
		...
		if ( count($font_fragments) >= 2 ){
			...
			/* Traditional Font */
			$stack = UpFrontFonts::get_stack($value);
	...
	public static function complex_property_shadow($args) {
		...
		global $upfront_complex_property_check;

		//If the complex property check isn't even set, make it an empty array.
		if ( !is_array($upfront_complex_property_check) )
			$upfront_complex_property_check = array($shadow_type => array());

		//Since the complex property is a combination of a bunch of properties, we only want it to output once.
		if ( isset($upfront_complex_property_check[$shadow_type][$selector]) && $upfront_complex_property_check[$shadow_type][$selector] == true )
			return;

		$upfront_complex_property_check[$shadow_type][$selector] = true;

		if ( !isset($properties[$shadow_type . '-color']) )
			return null;

		$shadow_color = upfront_format_color($properties[$shadow_type . '-color']);
		...
		$shadow_inset 	= (upfront_get($shadow_type . '-position', $properties, 'outside') == 'inset') ? ' inset' : null;
