...
class UpFrontElementProperties {
		...
		foreach ( $properties as $property_id => $value ) {
			...
			if ( upfront_get( 'complex-property', $property ) && is_callable( upfront_get( 'complex-property', $property ) ) ) {
				$output .= call_user_func(
					upfront_get( 'complex-property', $property ),
					array(
						'selector' => $selector,
			...
			if ( 'image' === upfront_get( 'type', $property ) && 'none' !== $value ) {
				$value = 'url( ' . $value . ' )';
			}
	...
	public static function complex_property_shadow( $args ) {
		...
		if ( isset( $upfront_complex_property_check[ $shadow_type ][ $selector ] ) && true === $upfront_complex_property_check[ $shadow_type ][ $selector ] ) {
			return;
		}
