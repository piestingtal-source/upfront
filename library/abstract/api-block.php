 ...
 */
abstract class UpFrontBlockAPI {
	...
	public $options_class 	= 'UpFrontBlockOptionsAPI';		
	...
	public function register() {
		...
		// If the UpFront blocks array doesn't exist, create it.
		if ( ! is_array( $upfront_block_types ) ) {
			$upfront_block_types = array();
		}

		/**
		 * Inline editable fields
		 * UpFrontAudioBlock::inline_editable
		 * example 1:
		...
		$upfront_block_types[ $this->id ] = array(
			'name'                         => $this->name,
		...
		add_action( 'upfront_register_elements', array( $this, 'setup_main_block_element' ) );
		...
		if ( method_exists( $this, 'setup_elements' ) ) {
			add_action( 'upfront_register_elements', array( $this, 'setup_elements' ) );
		}

		// Setup hooks.
		if ( $this->allow_titles ) {
			add_action( 'upfront_block_content_' . $this->id, array( $this, 'title_and_subtitle' ) );
		}

		add_action( 'upfront_block_content_' . $this->id, array( $this, 'content' ) );

		add_action( 'upfront_block_options_' . $this->id, array( $this, 'options_panel' ), 10, 2 );
	}
	...
	public function options_panel($block, $layout) {
		...
		if ( !class_exists($this->options_class) )
			return new WP_Error('block_options_class_does_not_exist', __('Fehler: Die registrierte Blockoptionsklasse existiert nicht.', 'upfront'), $this->options_class);
	...
	public function title_and_subtitle($block) {
		...
		/* Output Block Titles */
		if ( upfront_get('original', $block) )
			$block = upfront_get('original', $block);

		$block_settings          = upfront_get( 'settings', $block );		
		$block_title             = upfront_get( 'block-title', upfront_get( 'settings', $block, array() ) );
		$block_title_tag         = upfront_get( 'block-title-tag', upfront_get( 'settings', $block, array() ) );
		$block_subtitle          = upfront_get( 'block-subtitle', upfront_get( 'settings', $block, array() ) );
		$block_subtitle_tag      = upfront_get( 'block-subtitle-tag', upfront_get( 'settings', $block, array() ) );
		$block_title_link_url    = upfront_get( 'block-title-link-url', upfront_get( 'settings', $block, array() ) );
		$block_title_link_target = upfront_get( 'block-title-link-target', $block_settings, false, true ) ? $target = ' target="_blank"' : '';
		$block_title_link_rel 	 = upfront_get( 'block-title-link-rel', $block_settings, 'noreferrer' );
		...
		if ( $block_title || $block_subtitle ) {
			...
			/* Title */
			if ( $block_title ) {
				if ( upfront_get( 'block-title-link-check', $block_settings, false ) ) {
					if($block_title_tag == ''){
						echo '<h1 class="block-title"><a href="' . upfront_fix_data_type( $block_title_link_url ) . '"' . $block_title_link_target . ' rel="'.$block_title_link_rel.'"><span>' . upfront_fix_data_type( $block_title ) . '</span></a></h1>';
					}else{
						echo '<'.$block_title_tag.' class="block-title"><a href="' . upfront_fix_data_type( $block_title_link_url ) . '"' . $block_title_link_target . ' rel="'.$block_title_link_rel.'"><span>' . upfront_fix_data_type( $block_title ) . '</span></a></'.$block_title_tag.'>';
					}
				} else {
					if($block_title_tag == ''){
						echo '<h1 class="block-title"><span>' . upfront_fix_data_type( $block_title ) . '</span></h1>';
					}else{
						echo '<'.$block_title_tag.' class="block-title"><span>' . upfront_fix_data_type( $block_title ) . '</span></'.$block_title_tag.'>';
					}
				}
			}
			...
			if ( $block_subtitle ) {
				if($block_subtitle_tag == ''){
					echo '<h2 class="block-subtitle">' . upfront_fix_data_type( $block_subtitle ) . '</h2>';
				}else{
					echo '<'.$block_subtitle_tag.' class="block-subtitle">' . upfront_fix_data_type( $block_subtitle ) . '</'.$block_subtitle_tag.'>';				
				}
			}
	...
	public function register_block_element($args) {
		...
		foreach ( $selector_array as $selector_index => $selector ) {
			...
			if( strlen($selector) > 0 && $selector[0] === '\\' ){			
				...
				$args['tooltip'] = 'Warnung: Das Anwenden eines Stils auf dieses Element wirkt sich auf alle Instanzen aus';
			}else{
		...
		return UpFrontElementAPI::register_element($element);
	...
	public static function get_setting($block, $setting, $default = null) {
		...
		return UpFrontBlocksData::get_block_setting($block, $setting, $default);
