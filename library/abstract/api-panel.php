...
abstract class UpFrontVisualEditorPanelAPI {
	...
	public function register() {
		...
		remove_action('upfront_visual_editor_panel_top_tabs', array('UpFrontVisualEditorDisplay', 'add_default_panel_link'));
		remove_action('upfront_visual_editor_content', array('UpFrontVisualEditorDisplay', 'add_default_panel'));

		add_action('upfront_visual_editor_panel_top_tabs', array($this, 'panel_link'));
		add_action('upfront_visual_editor_content', array($this, 'build_panel'));
	...
	public function render_input( $input = array() ) {
		...
		/* Populate the value */
			...
			if ( isset($this->wrapper) && $this->wrapper && !isset($input['value']) )
				$input['value'] = UpFrontWrappersData::get_wrapper_setting($this->wrapper, $input['name'], $input['default']);

			else if ( isset($this->block) && $this->block && !isset($input['value']) )
				$input['value'] = UpFrontBlocksData::get_block_setting($this->block, $input['name'], $input['default']);

			else if ( !isset($input['value']) && upfront_get('template-option', $input, true) )
				$input['value'] = UpFrontSkinOption::get($input['name'], $input['group'], $input['default']);

			else if ( !isset($input['value']) )
				$input['value'] = UpFrontOption::get($input['name'], $input['group'], $input['default']);
	...
	public function repeater($input) {
		...
		$repeater_sortable_class = upfront_get('sortable', $input) ? ' repeater-sortable' : null;

		echo '<div class="repeater' . $repeater_sortable_class . '" data-repeater-limit="' . upfront_get('limit', $input, '0') . '">';

			$repeater_label = upfront_get('label', $input);
			if( !empty($repeater_label) ) {
				...
				$this->render_input(array(
					...
					'tooltip' => upfront_get('tooltip', $input)
				));
		...
		public function repeater_group($input, $group_index = null) {
			...
			if ( upfront_get('template', $input) )
				$classes[] = 'repeater-group-template';

			if ( upfront_get('single', $input) )
				$classes[] = 'repeater-group-single';


			echo '<div class="' . implode(' ', $classes) . '">';

				if ( upfront_get('sortable', $input) )
				...
				foreach ( $input['inputs'] as $index => $input_options ) {
					...
					$input_value = upfront_get($input_options['name'], $input_value_group_index, upfront_get('default', $input_options));
	...
	public function input_checkbox($input) {
		...
		$checked_attribute = ( (bool)upfront_fix_data_type($input['value']) === true ) ? ' checked="checked"' : null;
	...
	public function input_text($input) {
		...
		echo '
			...
			<div class="input-right">
				<input type="text" ' . $input['attributes'] . ' placeholder="' . stripslashes(esc_attr(upfront_get('placeholder', $input))) . '" value="' . stripslashes(esc_attr($input['value'])) . '" class="text"' . $readonly . ' />';
	...
	public function input_textarea($input) {
		...
		echo '
			...
			<div class="input-right">
				<span class="textarea-open pencil-icon tooltip" title="' . __('Textbereich anzeigen', 'upfront') . '"></span>
	...
	public function input_code($input) {
			...
			echo '
				...
				<div class="input-right">
					<span class="code-editor-open pencil-icon tooltip" title="' . __('Code-Editor anzeigen', 'upfront') . '" data-editor-mode="' . upfront_get('mode', $input, 'php') . '"></span>
					<textarea ' . $input['attributes'] . '>' . stripslashes(esc_textarea($input['value'])) . '</textarea>
				</div>
			';
	...
	public function input_wysiwyg($input) {
		...
		echo '
			...
			<div class="input-right">
				<span class="wysiwyg-open pencil-icon tooltip" title="' . __('Editor anzeigen', 'upfront') . '"></span>
	...
	public function input_select($input) {
		...
		$chosen_class = ( upfront_get('chosen', $input) ) ? ' select-chosen' : '';
	...
	public function input_multi_select($input) {
		...
		echo '<div class="input-right">';
			...
			echo '<span class="multi-select-open pencil-icon tooltip" title="Optionen anzeigen"></span>';
			echo '<div class="multi-select-container">';

				echo '<select ' . $input['attributes'] . ' multiple="multiple" class="tooltip" title="' . __('Halte Strg (Windows) oder Befehl (Mac) gedrückt, um mehrere Optionen auszuwählen.', 'upfront') . '">';
	...
	public function input_colorpicker($input) {
		...
		$input['value'] = upfront_format_color($input['value']);
	...
	public function input_image($input) {
		...
		<span class="delete-image"' . $src_visibility . '>Löschen</span>';

		echo '<span class="button">' . __('Wähle Bild', 'upfront') . '</span>
	...
	public function input_audio($input) {
		...
		echo '<div class="input-right"><span class="src"' . $src_visibility . '>' . $filename . '</span>
		<span class="delete-audio"' . $src_visibility . '>Löschen</span>';

		echo '<span class="button">' . __('Wähle Audio', 'upfront') . '</span>';
	...
	public function input_video($input) {
		...
		echo '<div class="input-right"><span class="src"' . $src_visibility . '>' . $filename . '</span>
		<span class="delete-video"' . $src_visibility . '>Löschen</span>';

		echo '<span class="button">' . __('Wähle Video', 'upfront') . '</span>';
		echo '<input ' . $input['attributes'] . ' type="hidden" value="' . $input['value'] . '" /></div><!-- .input-right -->';
	...
	public function input_import_file($input) {
		...
		echo '<div class="input-right">';
			...
			<span class="delete-file"' . $src_visibility . '>' . __('Löschen', 'upfront') . '</span>';
	...
	public function input_json($input) {
		...
		echo '<div class="input-right"><span class="src"' . $src_visibility . '>' . $filename . '</span>
		<span class="delete-json"' . $src_visibility . '>Löschen</span>';

		echo '<span class="button">' . __('Wähle Json', 'upfront') . '</span>';
		echo '<input ' . $input['attributes'] . ' type="hidden" value="' . $input['value'] . '" /></div><!-- .input-right -->';
