...
abstract class UpFrontWebFontProvider {
	...
	public function __construct() {
		...
		$this->transient_id = 'upfront_webfonts_' . $this->id;

		add_action('upfront_fonts_browser_tabs', array($this, 'tab'));
		add_action('upfront_fonts_browser_content', array($this, 'content'));

		if ( $this->load_with_ajax )
			add_action('upfront_fonts_ajax_list_fonts_' . $this->id, array($this, 'list_fonts'));
	...
	public function content_search() {
		...
		echo '<form class="fonts-search">';
	      	...
	      	echo '<input class="fonts-filter" type="text" placeholder="' . __('Suche ', 'upfront') . $this->name . '" name="search-input">';

	      	if ( is_array($this->sorting_options) && !empty($this->sorting_options) ) {

	      		echo '<div class="select-container"><select name="choices">';
	      			echo '<option value="" disabled="disabled">&ndash; ' . __('Sortieren nach ', 'upfront') . '&ndash;</option>';
	...
	public function content_fonts_list() {
		...
		echo '
			<div class="fonts-list webfonts-list">
				...
				<div class="loading fonts-loading"><p>' . __('Lade Schriftarten...', 'upfront') . '</p></div>
				<div class="fonts-noresults" style="display:none;">' . __('Keine Ergebnisse', 'upfront') . '</div>
			</div><!-- .fonts-list -->
		';
	...
	public function list_fonts($sortby = false) {
		...
		if ( upfront_post('sortby') )
			$sortby = upfront_post('sortby');

		$fonts = $this->retrieve_fonts($sortby);

		if ( !$fonts ) {
			echo __('<p class="error">Derzeit können keine Schriftarten abgerufen werden.</p>', 'upfront');
			return;
		}
		...
		foreach ( $fonts as $font ) {
			...
			$html .= ']">
					...
					<span class="font-preview-text">' . __('Der schnelle Braunfuchs springt über den faulen Hund.', 'upfront') . '</span> 

					<span title="Schrift verwenden" class="use-font action"></span>
					<span title="Schriftvorschau" class="preview-font action"></span>
				</li>';
