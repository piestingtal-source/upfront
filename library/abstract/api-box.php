...
abstract class UpFrontVisualEditorBoxAPI {
	...
	public function register() {
		...
		global $upfront_registered_ve_boxes;

		if ( !isset($upfront_registered_ve_boxes) )
			$upfront_registered_ve_boxes = array();

		if ( in_array($this->id, $upfront_registered_ve_boxes) )
			return false;

		$mode = UpFrontVisualEditor::get_current_mode();

		if ( $this->mode === 'all' || strtolower($this->mode) === strtolower($mode) )
			add_action('upfront_visual_editor_boxes', array($this, 'build_box'));		

		if ( $this->load_with_ajax )
			add_action('upfront_visual_editor_ajax_box_content_' . $this->id, array($this, 'content'));

		$upfront_registered_ve_boxes[] = $this->id;
