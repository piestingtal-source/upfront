<?php
/**
 * UpFront Theme main function file
 *
 * @since 1.0.0
 ...
 */
abstract class UpFrontAdminMetaBoxAPI {
	...
	public function register() {
		...
		$this->post_type_supports_id = isset( $this->post_type_supports_id ) ? $this->post_type_supports_id : 'upfront-admin-meta-box-' . $this->id;

		/* Add the prefix to the ID */
		$this->id = 'upfront-admin-meta-box-' . $this->id;
	...
	public function box() {
		...
		echo '<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table upfront-admin-meta-box">';
		...
		foreach ( $this->inputs as $name => $input ) {
			...
			$input['value'] = UpFrontLayoutOption::get( $post->ID, $input['id'], $input['default'], $global, $input['group'] );

			if ( upfront_get( 'name', $input ) ) {
	...
	public function save( $post_ID ) {
		...
		/* Ruh-roh, bad nonce */
		if ( ! wp_verify_nonce( upfront_post( $this->id . '_nonce' ), md5( $this->id ) ) ) {
			return false;
		}
		...
		foreach ( $_POST[ $this->id ] as $group => $inputs ) {
			...
			foreach ( $inputs as $input => $value ) {
				...
				UpFrontLayoutOption::set( $post_ID, $input, $value, $global, $group );

				if ( 'template' === $input ) {
					UpFrontLayout::clear_status_transient();
				}
			}
		}
	}
	...
	protected function input_select( $input ) {
		...
		if ( upfront_get( 'blank-option', $input ) ) {
			echo '<option value="">' . upfront_get( 'blank-option', $input ) . '</option>';
		}
