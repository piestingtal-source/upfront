...
abstract class UpFrontNotice {
	...
	public static function notice( $notice_id, $template_file ){
		...
		self::$option_name      = 'upfront_option_dismissed_notices';
	...
	public static function dismiss_notice() {
		...
		if ( ! wp_verify_nonce( $_GET['nonce'], 'upfront_dismiss_notice' ) ) {
			return;
		}
	...
	protected static function get_dismiss_link( $defer = false, $redirect = '' ) {
		...
		$link = admin_url(
			sprintf(
				...
				'upfront_dismiss_notice',
				wp_create_nonce( 'upfront_dismiss_notice' ),
				self::$notice_id
			)
		);
