function openTabAdmin(evt, option) {
    var i, x, tablinks;
    x = document.getElementsByClassName("upfront-admin-tab");
    for (i = 0; i < x.length; i++) {
        x[i].style.display = "none";
    }
    tablinks = document.getElementsByClassName("tablink");
    for (i = 0; i < x.length; i++) {
        tablinks[i].className = tablinks[i].className.replace(" upfront-admin-border-red", "");
    }
    document.getElementById(option).style.display = "block";
    evt.currentTarget.firstElementChild.className += " upfront-admin-border-red";
}
(function($) {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        showNotification = function(args) {
            ...
            var notification = $('<div id="upfront-notification-' + args.id + '" class="updated below-h2"><p>' + args.message + '</p></div>');

            if ($('#upfront-admin-notifications').length == 0) {
                $('<div id="upfront-admin-notifications"></div>').appendTo('body');
            }

            notification.appendTo('#upfront-admin-notifications');
        ...
        showErrorNotification = function(args) {
            ...
            var notification = $('<div id="upfront-notification-' + args.id + '" class="error below-h2"><p>' + args.message + '</p></div>');
        ...
        updateNotification = function(id, message) {
            ...
            return $('#upfront-notification-' + id).children('p').html(message);

        }

        hideNotification = function(id) {

            $('#upfront-notification-' + id).fadeOut(500);

        }

        /* Notice */
        $('[data-upfront-notice]').on('click', '.upfront-dismiss-notice, .notice-dismiss', function() {

            var $notice = $(this).closest('.notice');
            var $noticeDismissWP = $notice.find('.notice-dismiss');
            var $noticeDismissBT = $notice.find('.upfront-dismiss-notice');

            var noticeID = $notice.data('upfront-notice');

            if ($(this).hasClass('upfront-dismiss-notice') && $noticeDismissWP.length) {
                $noticeDismissWP.trigger('click');
            } else if ($(this).hasClass('upfront-dismiss-notice')) {
            ...
            return $.ajax({
                ...
                data: {
                    'action': 'upfront_dismiss_admin_notice',
        ...
        $('#upfront-responsive-grid-notice').on('click', '.button-primary', function() {

            var $notice = $(this).closest('.notice');
            var message = 'Bitte beachte: Wenn Du Probleme mit dem Responsive Gitter hast, kannst Du es im Visual Editor im Gitter-Modus deaktivieren.';
            ...
            return $.ajax({
                ...
                timeout: 10000, // throw an error if not completed after 30 sec.
                data: {
                    'action': 'upfront_enable_responsive_grid'
                }
            });
        ...
        */
        if (jQuery('.upfront-admin-row.menu').length > 0) {
            var distance = jQuery('.upfront-admin-row.menu').offset().top,
                $window = jQuery(window);

            $window.scroll(function() {
                if ($window.scrollTop() >= distance) {
                    jQuery('.upfront-admin-row.menu').addClass('sticky')
                }
                if ($window.scrollTop() == 0) {
                    jQuery('.upfront-admin-row.menu').removeClass('sticky')
                }
            });
        }
        ...
        /*
        	Clear cache
        */
        jQuery(document).on('click', '#wp-admin-bar-upfront-admin-tools-clear-cache', function(event) {
        ...
        */
        $(document).on('click', 'button.handlediv', function() {
            $(this).closest('.upfront-admin-options-group').toggleClass('closed');
        })
    });
})(jQuery);