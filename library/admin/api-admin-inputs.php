...
class UpFrontAdminInputs {	
	...
	public static function generate($inputs, $table_class = 'form-table') {
		...
		echo '<table class="' . $table_class . '">';
			...
			foreach ( $inputs as $input ) {
				...
				if ( upfront_get('value', $input) && !is_int(upfront_get('value', $input)) )
	...
	public static function admin_field_generate($inputs, $class = 'form-table') {
		...
		echo '<div class="inside upfront-fields left '.$class.'">';
		foreach ( $inputs as $input ) {
			...
			echo '<div class="upfront-field">';
			echo 	'<div class="upfront-label">';
			echo 		'<label>' . $input['label'] . $tooltip . '</label>';
			echo 		'<p class="description">' . $suffix . $description . '</p>';			
			echo 	'</div>';
			echo 	'<div class="upfront-input">';
	...
	public static function input_text($input) {
		...
		$name_attr = $input['no-submit'] ? null : ' name="upfront-admin-input[' . $input['id'] . ']"';
	...
	public static function input_checkbox($input) {
		...
		echo '<fieldset>';
				...
				foreach($input['checkboxes'] as $checkbox) {
					...
					$name_attr = $input['no-submit'] ? null : ' name="upfront-admin-input[' . $checkbox['id'] . ']"';
	...
	public static function input_select($input) {
		...
		$name_attr = $input['no-submit'] ? null : ' name="upfront-admin-input[' . $input['id'] . ']' . $name_multiple_array . '"';
		...
		if ( !$input['no-submit'] && $input['multiple'] )
			echo '<input type="hidden" value="0" name="upfront-admin-input[' . $input['id'] . ']" />';
