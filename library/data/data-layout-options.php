...
class UpFrontLayoutOption {
	...
	public static function is_post($layout) {
		...
		$layout_fragments = explode(UpFrontLayout::$sep, $layout);
	...
	public static function get($layout, $option, $default = null, $global = false, $group_prefix = false) {
		...
		/* Otherwise use the UpFront layout meta table */
		} else {
		...
		return upfront_fix_data_type($return);
		...
		/* Otherwise use the UpFront layout meta table */
			...
			if ( $meta_id = $wpdb->get_var($wpdb->prepare("SELECT meta_id FROM $wpdb->pu_layout_meta WHERE layout = '%s' AND template = '%s' AND meta_key = '%s'", $layout, $template, $option)) ) {
				...
				return $wpdb->update($wpdb->pu_layout_meta, array(
					'meta_value' => upfront_maybe_serialize($value)
				), array(
					'meta_id' => $meta_id
				));
