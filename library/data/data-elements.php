...
class UpFrontElementsData {
	...
	public static function init() {
		...
		add_action('upfront_visual_editor_save', array(__CLASS__, 'merge_core_default_design_data'));
	...
	public static function get_raw_data($defaults = array()) {
		...
		self::$raw_data = upfront_array_merge_recursive_simple(self::get_legacy_default_data(), UpFrontSkinOption::get('properties', 'design', $defaults));
	...
	public static function get_element_properties($element) {
		...
		$element = upfront_get($element_id, self::get_raw_data());
	...
	public static function get_special_element_properties($args) {
		...
		$element = upfront_get($element_id, self::get_raw_data(), array(
	...
	public static function get_property($element_id, $property_id, $default = null, $element_group = null) {
		...
		if ( $properties !== null && !is_wp_error($properties) && isset($properties[$property_id]) && (upfront_fix_data_type($properties[$property_id]) || upfront_fix_data_type($properties[$property_id]) === 0) )
	...
	public static function set_property($element_group = null, $element_id, $property_id, $value) {
		...
		$all_properties = UpFrontSkinOption::get('properties', 'design', array());
		...
		/* Send it back to DB */
		return UpFrontSkinOption::set('properties', $all_properties, 'design');
	...
	/* JS Properties */
		public static function set_js_property($element_id, $js_property_id, $options) {
			...
			$all_js_properties = UpFrontSkinOption::get( 'js-properties', 'design', array() );
			...
			/* Send it back to DB */
			return UpFrontSkinOption::set( 'js-properties', $all_js_properties, 'design' );
	...
	/* Defaults */
		public static function get_default_data() {
			...
			global $upfront_core_default_element_data;

			return $upfront_core_default_element_data;
			...
			public static function get_legacy_default_data() {
				...
				if ( !isset($upfront_default_element_data) || !is_array($upfront_default_element_data) )
					$upfront_default_element_data = array();
		...
		/**
		 * Merge in default design data.  This will be ran upon save and upgrade to UpFront 1.0.0
		 */
		public static function merge_core_default_design_data() {

			self::merge_default_design_data( UpFrontElementsData::get_default_data(), 'core' );
			...
			/* This function accepts data as well as ID that way it can be used by UpFront plugins */
			public static function merge_default_design_data($default_data, $id) {
				...
				if ( !UpFrontSkinOption::get($merge_id, 'general', false) ) {

					$design_data = UpFrontSkinOption::get('properties', 'design', array());
					$design_data_with_defaults = upfront_array_merge_recursive_simple($default_data, $design_data);
					
					UpFrontSkinOption::set($merge_id, true, 'general');

					return UpFrontSkinOption::set('properties', $design_data_with_defaults, 'design');
