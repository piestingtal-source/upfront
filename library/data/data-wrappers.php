...
class UpFrontWrappersData {
	...
	public static function add_wrapper($layout_id, $args) {
		...
		$mirror_id = upfront_get('mirror-wrapper', upfront_get('settings', $args, array()));
	...
	public static function update_wrapper($wrapper_id, $args) {
		...
		/* Query */
		$query = $wpdb->update($wpdb->pu_wrappers, array_map('upfront_maybe_serialize', $args), array(
			'template' => $template,
			'id' => $wrapper_id
		));
	...
	public static function delete_wrapper($layout_id, $wrapper_id) {
		...
		$blocks_delete_query = UpFrontBlocksData::delete_by_wrapper($layout_id, $wrapper_id);
	...
	public static function delete_wrapper_design_instances($layout_id, $wrapper_id) {
		...
		return UpFrontElementsData::delete_special_element_properties(null, 'wrapper', 'instance', 'wrapper-' . UpFrontWrappers::format_wrapper_id($wrapper_id));
	...
	public static function get_wrapper($wrapper, $use_mirrored = false) {
		...
		if ( is_string($wrapper) || is_numeric($wrapper) ) {
			...
			$wrapper_id = UpFrontWrappers::format_wrapper_id($wrapper);

			$wrapper_query = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->pu_wrappers WHERE template = '%s' AND id = '%s'", UpFrontOption::$current_skin, $wrapper_id), ARRAY_A);

			if ( is_array($wrapper_query) && !is_wp_error($wrapper_query) ) {

				$wrapper = array_map('upfront_maybe_unserialize', $wrapper_query);

			} else {

				/* If no wrapper is found, try querying the legacy_id */
				$wrapper_from_legacy_id = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->pu_wrappers WHERE template = '%s' AND legacy_id = '%d'", UpFrontOption::$current_skin, $wrapper_id), ARRAY_A);

				if ( is_array($wrapper_from_legacy_id) && ! is_wp_error( $wrapper_from_legacy_id ) ) {
					$wrapper = array_map( 'upfront_maybe_unserialize', $wrapper_from_legacy_id );
				} else {
					$wrapper = null;
				}
	...
	public static function get_wrappers_by_layout($layout_id, $include_styling = false) {
		...
		if ( $layout_wrappers === false ) {
			...
			$layout_wrappers_query = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->pu_wrappers WHERE template = '%s' AND layout = '%s' ORDER BY position ASC", UpFrontOption::$current_skin, $layout_id), ARRAY_A);
			...
			foreach ( $layout_wrappers_query as $layout_wrapper ) {
				...
				$data = array_map('upfront_maybe_unserialize', $layout_wrapper);

				$layout_wrappers[$layout_wrapper['id']] = array_map('upfront_maybe_unserialize', $layout_wrapper);
			...
			if ( !count($layout_wrappers) ) {
				$layout_wrappers = UpFrontWrappers::$default_wrappers;
	...
	public static function get_all_wrappers($include_styling = false, $mirrored_only = false) {
		...
		$wrapper_query = $wpdb->get_results($wpdb->prepare($query, UpFrontOption::$current_skin), ARRAY_A);

		/* Change results array into associative */
		$wrappers = array();

		foreach ( $wrapper_query as $wrapper ) {
			$wrappers[$wrapper['id']] = array_map('upfront_maybe_unserialize', $wrapper);
		}
	...
	public static function add_wrapper_styling_to_array($wrappers) {
		...
		foreach ( $wrappers as $wrapper_id => $wrapper ) {
			...
			$wrappers[$wrapper_id]['styling'] = UpFrontElementsData::get_special_element_properties(array(
				'element' => 'wrapper',
				'se_type' => 'instance',
				'se_meta' => 'wrapper-' . UpFrontWrappers::format_wrapper_id($wrapper_id)
			));
	...
	public static function get_wrapper_styling($wrapper_id) {
		...
		return UpFrontElementsData::get_special_element_properties(array(
			'element' => 'wrapper',
			'se_type' => 'instance',
			'se_meta' => 'wrapper-' . UpFrontWrappers::format_wrapper_id($wrapper_id)
		));
	...
	public static function is_wrapper_mirrored($wrapper) {
		...
		if ( $mirrored_wrapper_id = upfront_get('mirror_id', $wrapper) )
	...
	public static function get_wrapper_setting($wrapper, $option_name, $default_value = null) {
		...
		if (!is_array($wrapper))
			$wrapper = UpFrontWrappersData::get_wrapper($wrapper);

		return upfront_get($option_name, upfront_get('settings', $wrapper, array()), $default_value);
	...
	public static function get_legacy_id( $wrapper ) {
		...
		return is_numeric( upfront_get( 'legacy_id', $wrapper ) ) && upfront_get( 'legacy_id', $wrapper ) > 0 ? upfront_get( 'legacy_id', $wrapper ) : $wrapper['id'];
