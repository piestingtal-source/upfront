...
class UpFrontOption {
	...
	public static function init() {
		...
		self::$current_skin = UpFrontTemplates::get_active_id();
	...
	public static function get($option = null, $group_name = false, $default = null, $main_site = false, $fix_data_type = true) {
		...
		return upfront_fix_data_type($group_data[$option]);
		...
		public static function get_group($group_name, $main_site = false) {
			...
			/* Query for the option group */
				$group_data = self::get_wp_option('upfront_option_group_' . $group_name, $main_site);
		...
		public static function format_wp_option($option) {
			...
			/* Format option name */
				if ( self::$is_skin_option )
					$option = str_replace('upfront_', 'pu_|template=' . self::$current_skin . '|_', $option);
		...
		public static function get_wp_option($option, $main_site = false) {
			...
			if ( isset($alloptions[$option]) ) {
				return upfront_maybe_unserialize($alloptions[$option]);
			} else {
	...
	public static function set($option = null, $value = null, $group_name = false) {
		...
		$group_option_name = self::format_wp_option('upfront_option_group_' . $group_name);
	...
	public static function delete($option = null, $group_name = false) {
		...
		$option_name = self::format_wp_option('upfront_option_group_' . $group_name);
