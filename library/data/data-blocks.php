...
class UpFrontBlocksData {
	...
	public static function add_block( $layout_id, $args ) {
		...
		$args['position']   = upfront_maybe_unserialize( $args['position'] );
		$args['dimensions'] = upfront_maybe_unserialize( $args['dimensions'] );
		$args['settings']   = upfront_maybe_unserialize( $args['settings'] );

		// Figure out mirror ID.
		$mirror_id = upfront_get( 'mirror-block', upfront_get( 'settings', $args, array() ) );
		...
		$insert_args = array(
			...
			'template'   => upfront_get( 'template', $args, UpFrontOption::$current_skin ),
			'layout'     => $layout_id,
			'type'       => $args['type'],
			'wrapper_id' => upfront_get( 'wrapper', $args ),
			'position'   => upfront_maybe_serialize( $args['position'] ),
			'dimensions' => upfront_maybe_serialize( $args['dimensions'] ),
			'settings'   => upfront_maybe_serialize( upfront_get( 'settings', $args, array() ) ),
			'mirror_id'  => $mirror_id,
		);

		// Is a pre-defined ID required?
		if ( $insert_id = upfront_get( 'insert_id', $args ) ) {
			$insert_args['id'] = $insert_id;
		}

		if ( $legacy_id = upfront_get( 'legacy_id', $args ) ) {
	...
	public static function update_block( $block_id, $args ) {
		...
		/* Query */
		$query = $wpdb->update($wpdb->pu_blocks, array_map('upfront_maybe_serialize', $args), array(
			'template' => $template,
			'id' => $block_id
		));
	...
	public static function delete_block($block_id) {
		...
		public static function delete_block_design_instances($block_id, $block_type) {
			...
			$block_element = UpFrontElementAPI::get_element('block-' . $block_type);
			...
			/* Find all block children element instances and queue them to be deleted */
				foreach ( UpFrontElementAPI::get_block_elements($block_type) as $element_id => $element_info )
			...
			/* Delete the instances now */
				...
				UpFrontElementsData::batch_delete_special_element_properties($batch_data);
	...
	public static function get_block($block_id_or_obj, $use_mirrored = false) {
		...
		if ( is_array( $block_id_or_obj) ) {
			...
			if ( !isset( $block_id_or_obj['id']) && !upfront_get('new', $block_id_or_obj, false) )
				return null;
		...
		/* Fetch the block based off of ID */
			...
			if ( $block_from_cache === false ) {
				...
				$block = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->pu_blocks WHERE template = '%s' AND id = '%s'", UpFrontOption::$current_skin, $block_id_or_obj), ARRAY_A);

				if ( is_array($block) && !is_wp_error($block) ) {

					$block = array_map('upfront_maybe_unserialize', $block);

				} else {

					/* If no block is found, try querying the legacy_id */
					$block_from_legacy_id = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->pu_blocks WHERE template = '%s' AND legacy_id = '%d'", UpFrontOption::$current_skin, $block_id_or_obj ), ARRAY_A );

					if ( is_array($block_from_legacy_id) && ! is_wp_error( $block_from_legacy_id ) ) {
						$block = array_map( 'upfront_maybe_unserialize', $block_from_legacy_id );
					} else {
						$block = null;
					}
	...
	public static function get_blocks_by_layout($layout_id, $wrapper_id = false, $include_design_editor_instances = false) {
		...
		if ( $layout_blocks === false ) {
			...
			/* Retrieve all blocks from layout */
				$query_string = $wpdb->prepare("SELECT * FROM $wpdb->pu_blocks WHERE layout = '%s' AND template = '%s'", $layout_id, UpFrontOption::$current_skin);
			...
			/* Change results array into associative */
				...
				foreach ( $layout_blocks_query as $layout_block ) {
					...
					$layout_blocks[$layout_block['id']] = array_map('upfront_maybe_unserialize', $layout_block);
	...
	public static function get_blocks_by_wrapper_id( $wrapper_id ) {
		...
		if ( $wrapper_blocks === false ) {
			...
			/* Change results array into associative */
				...
				foreach ( $wrapper_blocks_query as $wrapper_block ) {
					$wrapper_blocks[$wrapper_block['id']] = array_map('upfront_maybe_unserialize', $wrapper_block);
				}
		...
		public static function get_block_styling($block) {
			...
			do_action('upfront_before_get_block_styling');

			$block_element = UpFrontElementAPI::get_element('block-' . $block['type']);
	...
	public static function get_blocks_by_wrapper($layout_id, $wrapper_id) {
		...
		$layout_blocks = self::get_blocks_by_layout($layout_id);
		$layout_wrappers = UpFrontWrappersData::get_wrappers_by_layout($layout_id);
		$wrapper_blocks = array();
		...
		foreach ( $layout_blocks as $block_id => $block ) {
			...
			if ( count($layout_wrappers) === 1 && (upfront_get('wrapper_id', $block) === null || upfront_get('wrapper_id', $block) == 'wrapper-default') ) {
				$wrapper_blocks[$block_id] = $block;
			}
	...
	public static function get_blocks_by_type($type) {
		...
		if ( $blocks_from_cache === false ) {
			...
			$blocks_by_type_query = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->pu_blocks WHERE template = '%s' AND type = '%s'", UpFrontOption::$current_skin, $type), ARRAY_A);
			...
			foreach ( $blocks_by_type_query as $block ) {
				...
				$blocks_by_type[$block['id']] = array_map('upfront_maybe_unserialize', $block);
	...
	public static function get_all_blocks() {
		...
		if ( $blocks_from_cache === false ) {
			...
			$blocks_query = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->pu_blocks WHERE template = '%s'", UpFrontOption::$current_skin), ARRAY_A);
			...
			foreach ( $blocks_query as $block ) {
				...
				$all_blocks[$block['id']] = array_map('upfront_maybe_unserialize', $block);
	...
	public static function is_block_mirrored($block) {
		...
		$block_settings = upfront_get( 'settings', $block, array() );
		...
		if ( $mirrored_block_id = upfront_get('mirror_id', $block) )
	...
	public static function get_block_mirror($block) {
		...
		if ( $mirrored_block_id = self::is_block_mirrored($block) )
			return UpFrontBlocksData::get_block($mirrored_block_id);
	...
	public static function get_block_name( $block ) {
		...
		return upfront_get('alias', upfront_get('settings', $block, array()), $default_name);
	...
	public static function get_block_width($block) {
		...
		/* Fetch the wrapper that way we can get its Grid settings */
			$wrapper = UpFrontWrappersData::get_wrapper(upfront_get('wrapper_id', $block, 'wrapper-default'));
	...
	public static function get_legacy_id($block) {
		...
		return is_numeric( upfront_get( 'legacy_id', $block ) ) && upfront_get( 'legacy_id', $block ) > 0  ? upfront_get( 'legacy_id', $block ) : $block['id'];
