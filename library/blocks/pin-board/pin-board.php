<?php
if ( !class_exists('UpFrontPinBoardCoreBlock') ) {

	class UpFrontPinBoardCoreBlock extends UpFrontBlockAPI {
		...
		function __construct(){
			...
			$this->id = 'pin-board';
			$this->name = __('Pin Board', 'upfront');
			$this->description = __('Verwende diese Option, um Deine Inhalte in einem Mauerwerksraster wie Pinterest anzuzeigen.', 'upfront');
			$this->options_class = 'UpFrontPinBoardCoreBlockOptions';
		...
		public static function init() {
			...
			add_action('wp_ajax_upfront_pin_board_infinite_scroll', array(__CLASS__, 'infinite_scroll_content'));
			add_action('wp_ajax_nopriv_upfront_pin_board_infinite_scroll', array(__CLASS__, 'infinite_scroll_content'));
		...
		public static function enqueue_action($block_id, $block = false) {
			...
			/* CSS */
			wp_enqueue_style('upfront-pin-board', upfront_url() . '/library/blocks/pin-board/css/pin-board.css');

			/* JS */
			wp_enqueue_script('upfront-pin-board', upfront_url() . '/library/blocks/pin-board/js/pin-board.js', array('jquery'));

			/* Variables */
			wp_localize_script('upfront-pin-board', 'UpFrontPinBoard', array(
		...
		public function content($block) {
			...
			if ( upfront_post('isAjax') ) {
				$is_archive = upfront_post( 'isArchive' );
				$is_search 	= upfront_post( 'isSearch' );
			} else {
			...
			$columns 			= parent::get_setting($block, 'columns', 3);
			$approx_pin_width 	= (UpFrontBlocksData::get_block_width($block) / $columns);
			...
			/* Setup Query */
				if ( parent::get_setting( $block, 'mode', 'default' ) != 'custom' && ($is_archive || $is_search) ) {
					$query_args = upfront_post( 'wpQueryVars' ) ? json_decode(stripslashes(upfront_post('wpQueryVars')), ARRAY_A) : $wp_query->query_vars;
				} else {
					$query_args = array();
				}
				...
				/* Pagination */
					...
					$pin_board_ajax_paged 	= upfront_post( 'pinBoardAjaxPaged' );
			...
			/* Custom query overrides */
				...
				echo '<div class="pin-board" data-pin-board-ajax-paged="' . upfront_get('paged', $query_args, 1) . '" data-pin-board-mode="' . parent::get_setting( $block, 'mode', 'default' ) . '">' . "\n";
				...
				while ( $wp_query->have_posts() ) {
					...
					do_action('upfront_before_pin_board_pin_setup');
					...
					do_action('upfront_before_pin_board_pin_open');
					...
					echo '<div class="pin-board-pin ' . implode(' ', $pin_classes) . '" data-post-id="'. get_the_ID() .'">' . "\n";
						...
						do_action('upfront_after_pin_board_pin_open');
						...
						if(isset($block['custom-fields']['above']) && is_array($block['custom-fields']['above']) && count($block['custom-fields']['above'])>0){
							...
							echo '<div class="'. implode(' ', apply_filters('upfront_pin_board_pin_custom_fields_class', array('custom-fields', 'custom-fields-above') ) )  .'">';
							...
							foreach ($block['custom-fields']['above'] as $post_type => $custom_fields) {
								...
								foreach ($custom_fields as $field_name => $label) {
									...
									$group_tag = apply_filters('upfront_pin_board_pin_custom_fields_group_tag', 'div' );
									$label_tag = apply_filters('upfront_pin_board_pin_custom_fields_label_tag', 'label' );
									$field_tag = apply_filters('upfront_pin_board_pin_custom_fields_field_tag', 'div' );

									$custom_field_content = get_post_meta($post_id,$field_name,true);
									$custom_field_content = apply_filters('upfront_pin_board_pin_custom_fields_field_content', $custom_field_content );
						...
						/* Thumbnail */
							if ( has_post_thumbnail() && $show_images ) {
								...
								if ( $crop_images_vertically ) {
									...
									$thumbnail_url 		= upfront_resize_image($thumbnail_object[0], $thumbnail_width, $thumbnail_height);
									$full_image_url 	= $thumbnail_object[0];

								} else {

									$thumbnail_object 	= wp_get_attachment_image_src($thumbnail_id, 'full');
									$thumbnail_url 		= upfront_resize_image($thumbnail_object[0], $thumbnail_width);
									$full_image_url 	= $thumbnail_object[0];
								...
								do_action('upfront_before_pin_thumbnail');
								...
								do_action('upfront_after_pin_thumbnail');
						...
						/* Meta */
							if ( $show_author || $show_datetime || $show_categories  || $show_tags || $show_post_type) {
								...
								do_action('upfront_before_pin_meta');
								...
								do_action('upfront_after_pin_meta');

							}
						/* End Meta */

						/* Excerpts/Content */
								do_action('upfront_before_pin_content');
								...
								if(isset($block['custom-fields']['before-content']) && is_array($block['custom-fields']['before-content']) && count($block['custom-fields']['before-content'])>0){
									...
									echo '<div class="'. implode(' ', apply_filters('upfront_pin_board_pin_custom_fields_class', array('custom-fields', 'custom-fields-before-content') ) )  .'">';
								...
								if(isset($block['custom-fields']['below']) && is_array($block['custom-fields']['below']) && count($block['custom-fields']['below'])>0){
									...
									echo '<div class="'. implode(' ', apply_filters('upfront_pin_board_pin_custom_fields_class', array('custom-fields', 'custom-fields-below') ) )  .'">';
								...
								do_action('upfront_after_pin_content');
						...
						do_action('upfront_before_pin_board_pin_close');

					echo '</div>' . "\n\n";

					do_action('upfront_after_pin_board_pin_close');

					do_action('upfront_after_pin_board_pin_setup');
			...
			if ( parent::get_setting($block, 'paginate', true) || $infinite_scroll ) {
				...
				do_action('upfront_before_pin_board_pagination');
				self::pagination($wp_query, $infinite_scroll, parent::get_setting($block, 'enumerate', false));
				do_action('upfront_after_pin_board_pagination');
			}
			...
			public static function infinite_scroll_content() {
				...
				$block = UpFrontBlocksData::get_block(upfront_post('block'));

				if ( is_array($block) && $block['type'] == 'pin-board' ) {
					do_action( 'upfront_block_content_pin-board', $block );
				}
		...
		/**
		 * Register elements to be edited by the UpFront Design Editor
		 **/
		...
		public function setup_elements() {
			...
			$this->register_block_element(array(
				'id' 			=> 'pin-below-thumb',
				'name' 			=> __('Pin Below Thumb', 'upfront'),
				'selector' 		=> '.pin-board-pin .below-thumb',				
			));	

			$this->register_block_element(array(
				'id' 			=> 'pin-title',
				'name' 			=> __('Pin Title', 'upfront'),
			...
			$this->register_block_element(array(
				'id' 			=> 'pin-title link',
				'name' 			=> __('Pin Title Link', 'upfront'),
			...
			$this->register_block_element(array(
				'id' 			=> 'pagination-text',
				'name' 			=> __('Pagination Current Page', 'upfront'),
			...
			$this->register_block_element(array(
				'id' => 'custom-fields',
				'name' => __('Custom Fields Container', 'upfront'),
				'selector' => '.custom-fields',			
			));
			$this->register_block_element(array(
				'id' => 'custom-fields-group',
				'name' => __('Custom Fields Group', 'upfront'),
				'selector' => '.custom-fields-group',
			));

				$this->register_block_element(array(
					'id' => 'custom-fields-div',
					'name' => __('Custom Fields Image', 'upfront'),
					'selector' => '.custom-fields image',			
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-div',
					'name' => __('Custom Fields Div', 'upfront'),
					'selector' => '.custom-fields div',			
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-p',
					'name' => __('Custom Fields text', 'upfront'),
					'selector' => '.custom-fields p',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-a',
					'name' => __('Custom Fields Link', 'upfront'),
					'selector' => '.custom-fields a',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-h1',
					'name' => __('Custom Fields H1', 'upfront'),
					'selector' => '.custom-fields h1',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-h2',
					'name' => __('Custom Fields H2', 'upfront'),
					'selector' => '.custom-fields h2',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-h3',
					'name' => __('Custom Fields H3', 'upfront'),
					'selector' => '.custom-fields h3',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-h4',
					'name' => __('Custom Fields H4', 'upfront'),
					'selector' => '.custom-fields h4',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-h5',
					'name' => __('Custom Fields H5', 'upfront'),
					'selector' => '.custom-fields h5',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-h6',
					'name' => __('Custom Fields H6', 'upfront'),
					'selector' => '.custom-fields h6',
				));

				$this->register_block_element(array(
					'id' => 'custom-fields-span',
					'name' => __('Custom Fields span', 'upfront'),
		...
		private static function pagination($query, $infinite_scroll = true, $enumerate) {
			...
			private static function previous_posts_link($label) {
				...
				if ( null === $label )
					$label = __( '&laquo; Previous Page', 'upfront' );
	...
	class UpFrontPinBoardCoreBlockOptions extends UpFrontBlockOptionsAPI {
		...
		function __construct($block_type_object){
			...
			$this->tabs = array(
				'pin-setup' 		=> __('Pin Setup', 'upfront'),
				'query-filters' 	=> __('Query Filters', 'upfront'),
				'pagination' 		=> __('Pagination/Infinite Scroll', 'upfront'),
				'text' 				=> __('Text', 'upfront'),
				'meta' 				=> __('Meta', 'upfront'),
				'images' 			=> __('Images', 'upfront'),
				'custom-fields'		=> __('Custom Fields', 'upfront'),
				'effects' 			=> __('Effects', 'upfront'),
			...
			$this->inputs = array(
				'pin-setup' => array(
					'mode' => array(
						'type' 		=> 'select',
						'name' 		=> 'mode',
						'label' 	=> __('Mode', 'upfront'),
						'tooltip' 	=> __('If you would like to modify the default behaviour, select custom query. <br/><strong>Note:</strong>On archive pages, it\'s not advisable to use a custom query if the block is displaying the archive results.<br/>On search pages, it may be necessary to limit results to only certain content types.', 'upfront'),
						'options' 	=> array(
								'default' 	=> __('Default behaviour', 'upfront'),
								'custom' 	=> __('Custom query', 'upfront'),
							),
						'toggle' 	=> array(
							'default' => array(
					...
					'columns' => array(
						...
						'slider-interval'	=> 1,
						'default' 			=> 3,
						'tooltip' 			=> __('Set how many pins to display horizontally', 'upfront')
					),

					'columns-smartphone' => array(
						'type' 				=> 'slider',
						'name' 				=> 'columns-smartphone',
						'label' 			=> __('Columns (iPhone/Smartphone)', 'upfront'),
						'slider-min' 		=> 1,
						'slider-max' 		=> 7,
						'slider-interval' 	=> 1,
						'default' 			=> 2,
						'tooltip' 			=> __('Set how many pins to display horizontally for iPhones and smartphones.  <strong>Recommended setting: 1 or 2</strong>', 'upfront')
					),

					'gutter-width' => array(
						'type' 				=> 'slider',
						'name' 				=> 'gutter-width',
						'label' 			=> __('Gutter Width', 'upfront'),
						'slider-min' 		=> 0,
						'slider-max' 		=> 100,
						'slider-interval' 	=> 1,
						'default' 			=> 15,
						'unit' 				=> 'px',
						'tooltip' 			=> __('The amount in space between pins horizontally.', 'upfront')
					),

					'pin-bottom-margin' => array(
						'type' 				=> 'slider',
						'name' 				=> 'pin-bottom-margin',
						'label' 			=> __('Pin Bottom Margin', 'upfront'),
						'slider-min' 		=> 0,
						'slider-max' 		=> 50,
						'slider-interval' 	=> 1,
						'default' 			=> 15,
						'unit' 				=> 'px',
						'tooltip' 			=> __('The amount of space on the bottom of each pin.', 'upfront')
					)
				),
				...
				'query-filters' => array(
					'pins-per-page' => array(
						'type' 		=> 'integer',
						'name' 		=> 'pins-per-page',
						'label' 	=> __('Pins Per Page', 'upfront'),
						'default' 	=> 10,
						'tooltip' 	=> __('Determines how many pins to load at one time before loading more via pagination or <em>infinite scrolling</em>.', 'upfront')
					),

					'offset' => array(
						'type' 		=> 'integer',
						'name' 		=> 'offset',
						'label' 	=> __('Offset', 'upfront'),
						'tooltip' 	=> __('The offset is the number of entries or posts you would like to skip.  If the offset is 1, then the first post will be skipped.', 'upfront'),
						'default' 	=> 0
					),
					...
					'filters-heading' => array(
						...
						'type' 	=> 'heading',
						'label' => __('Filters', 'upfront'),
					),

					'post-type' => array(
						'type' 		=> 'multi-select',
						'name' 		=> 'post-type',
						'label' 	=> __('Post Type', 'upfront'),
						'tooltip' 	=> __('Choose a post type to display. If none are selected, it will automatically default to all.', 'upfront'),
						'default' 	=> 'any',
					...
					'taxonomies' => array(
						'type' 		=> 'select',
						'name' 		=> 'taxonomies',
						'label' 	=> __('Taxonomy', 'upfront'),
						'default' 	=> 'category',
						'options' 	=> array( 'none' => 'No taxonomies' ),
						'tooltip' 	=> __('Select the taxonomy to filter pins on .', 'upfront'),
					...
					'categories' => array(
						...
						'tooltip' 	=> __('Filter the pins that are shown by the selected taxonomy\'s terms.', 'upfront')
					),
					'categories-mode' => array(
						'type' 		=> 'select',
						'name' 		=> 'categories-mode',
						'label' 	=> __('Terms Mode', 'upfront'),
						'tooltip' 	=> '',
						'options' 	=> array(
							'include' => __('Include', 'upfront'),
							'exclude' => __('Exclude', 'upfront')
						),
						'tooltip' 	=> __('If this is set to <em>include</em>, then only the pins that match the terms filter will be shown.  If set to <em>exclude</em>, all pins that match the selected terms will not be shown.', 'upfront')
					),
					'author' => array(
						'type' 		=> 'multi-select',
						'name' 		=> 'author',
						'label' 	=> __('Author', 'upfront'),
						'tooltip' 	=> '',
						'options' 	=> 'get_authors()'
					),
					'exclude-current-post' => array(
						...
						'label' => __('Exclude Current Post', 'upfront'),
						'default' => false,
						'tooltip' => __('Enabling this option will exclude the current Post from the pinboard, usefull when are creating a "related post" block.', 'upfront')
					),

					'order-heading' => array(
						'name' 	=> 'order-heading',
						'type' 	=> 'heading',
						'label' => __('Order', 'upfront'),
					),
					'order-by' => array(
						'type' 		=> 'select',
						'name' 		=> 'order-by',
						'label' 	=> __('Order By', 'upfront'),
						'tooltip' 	=> '',
						'options' 	=> array(
							'date' 	=> __('Date', 'upfront'),
							'title' => __('Title', 'upfront'),
							'rand' 	=> __('Random', 'upfront'),
							'ID' 	=> 'ID'
						)
					),
					'order' => array(
						...
						'tooltip' => '',
						'options' => array(
							'desc' => __('Descending', 'upfront'),
							'asc' => __('Ascending', 'upfront'),
						)
					)
				),
				...
				'pagination' => array(
					'paginate' => array(
						'type' => 'checkbox',
						'name' => 'paginate',
						'label' => __('Paginate Pins', 'upfront'),
						'default' => true,
						'tooltip' => __('Enabling pagination adds buttons to the bottom of the pin board to go to the next/previous page.  <strong>Note:</strong> If infinite scrolling is enabled, pagination will be hidden.', 'upfront')
					),

					'enumerate' => array(
						'type' => 'checkbox',
						'name' => 'enumerate',
						'label' => __('Enumerate pagination', 'upfront'),
						'default' => false,
						'tooltip' => __('If pagination is displayed, enabling this will also show page number navigation.', 'upfront')
					),

					'infinite-scroll' => array(
						'type' => 'checkbox',
						'name' => 'infinite-scroll',
						'label' => __('Infinite Scrolling', 'upfront'),
						'default' => true,
						'tooltip' => __('Infinite scrolling allows your visitors to view all of your pins without the need for them to click a button to continue to the next page.  The pins will be loaded automatically simply by scrolling.', 'upfront')
					)
				),
				...
				'text' => array(
					...
					'titles-position' => array(
							...
							'label' => __('Titles position', 'upfront'),
							'default' => 'below',
							'options' => array('above' => __('Above', 'upfront'),'below' => __('Below', 'upfront') )
						),

					'titles-link-to-post' => array(
							'type' => 'checkbox',
							'name' => 'titles-link-to-post',
							'label' => __('Titles link to post', 'upfront'),
							'default' => true,
							'tooltip' => __('Open the post when the user clicks on the title', 'upfront')
						),

					'content-to-show' => array(
							'type' => 'select',
							'name' => 'content-to-show',
							'label' => __('Content To Show', 'upfront'),
							'options' => array(
								'' => __('&ndash; Do Not Show Content &ndash;', 'upfront'),
								'excerpt' => __('Excerpts', 'upfront'),
								'content' => __('Full Content', 'upfront')
							),
							'default' => 'excerpt',
							'tooltip' => __('The content is the written text or HTML for the entry.  This is edited in the WordPress admin panel.', 'upfront')
						),

					'show-text-if-no-image' => array(
							'type' => 'checkbox',
							'name' => 'show-text-if-no-image',
							'label' => __('Only show content when no featured image', 'upfront'),
							'default' => false,
							'tooltip' => __('If enabled, regardless of the content chosen in <em>Content to Show</em> will only show content for pins with no featured image.', 'upfront')
						),
				...
				'meta' => array(
					...
					'show-author' 	=> array(
							'type' 		=> 'checkbox',
							'name' 		=> 'show-author',
							'label' 	=> __('Meta: Show Author "byline"', 'upfront'),
							'default' 	=> false,
							'tooltip' 	=> __('<strong>Example:</strong> <em>by</em> Author Name', 'upfront')
						),

					'show-category' => array(
							'type' 		=> 'checkbox',
							'name' 		=> 'show-category',
							'label' 	=> __('Meta: Show Categories', 'upfront'),
							'default' 	=> false
						),
					...
					'show-tags' => array(
							'type' 		=> 'checkbox',
							'name' 		=> 'show-tags',
							'label' 	=> __('Meta: Show Tags', 'upfront'),
							'default' 	=> false
						),
					...
					'show-post-type' => array(
							'type' 		=> 'checkbox',
							'name' 		=> 'show-post-type',
							'label' 	=> __('Meta: Show Post Type', 'upfront'),
							'default' 	=> false
						),
					...
					'show-datetime' => array(
							'type' 		=> 'checkbox',
							'name' 		=> 'show-datetime',
							'label' 	=> __('Meta: Show Date/Time', 'upfront'),
							'default' 	=> false
						),
				...
				'images' => array(
					'show-images' => array(
						'type' => 'checkbox',
						'name' => 'show-images',
						'label' => __('Show Images', 'upfront'),
						'default' => true,
					),
					'images-click-action' => array(
						'type' => 'select',
						'name' => 'image-click-action',
						'label' => __('Image click action', 'upfront'),
						'default' => 'link',
						'tooltip' => __('Choose the action when user clicks on an image.', 'upfront'),
						'options' => array(
							'post'  => __('Open post', 'upfront'),
							'popup' => __('Popup original image', 'upfront'),
							'none'  => __('Do nothing', 'upfront')
						)
					),
					...
					'crop-vertically' => array(
						...
						'label' => __('Crop Vertically', 'upfront'),
						'default' => false,
						'tooltip' => __('Trim all images to have the same height.  The trimmed/cropped height is roughly 75% of the width.', 'upfront')
					)
				),
				...
				'effects' => array(
					'hover-focus' => array(
						'type' => 'checkbox',
						'name' => 'hover-focus',
						'label' => __('Hover Focus', 'upfront'),
						'default' => false,
						'tooltip' => __('If enabled, the hovered pin will be focused while all others will be faded out.', 'upfront')
					)
				),
				...
				'social' => array(
					'show-pinterest-button' => array(
						...
						'label' => __('Pinterest: Show "Pin It" Button', 'upfront'),
						'default' => false,
						'tooltip' => __('Show a Pinterest "Pin It" button inside of the images.', 'upfront'),
					),

					'show-twitter-button' => array(
						'type' => 'checkbox',
						'name' => 'show-twitter-button',
						'label' => __('Twitter: Tweet-Button anzeigen', 'upfront'),
						'default' => false,
						'tooltip' => __('Zeige einen Tweet-Button entweder innerhalb des Beitrags-Bildes oder nach dem Titel an.', 'upfront'),
					),

					'twitter-username' => array(
						'type' => 'text',
						'name' => 'twitter-username',
						'label' => __('Twitter: Dein Benutzername', 'upfront')
					),

					'twitter-hashtag' => array(
						'type' => 'text',
						'name' => 'twitter-hashtag',
						'label' => __('Twitter: Hashtag zum Einfügen von Tweets (optional)', 'upfront')
					),

					'show-facebook-button' => array(
						'type' => 'checkbox',
						'name' => 'show-facebook-button',
						'label' => __('Facebook: Show Like/Share Button', 'upfront'),
						'default' => false,
						'tooltip' => __('Zeige einen Facebook-Share/Like-Button entweder innerhalb des Beitrags-Bildes oder nach dem Titel an.', 'upfront'),
					),

					'facebook-button-verb' => array(
						'type' => 'select',
						'label' => __('Facebook: Button Verb', 'upfront'),
						'name' => 'facebook-button-verb',
						'options' => array(
							'like' => __('Like', 'upfront'),
							'recommend' => __('Empfehlen', 'upfront')
						),
						'default' => 'like'
					)
				)
		...
		public function modify_arguments($args = false) {
			...
			$tax_slug 			= UpFrontBlockAPI::get_setting($block, 'taxonomies', 'category');
			...
			$this->inputs['query-filters']['categories'] = array(
				...
				'tooltip' 	=> __('Filtere die Pins, die in den Begriffen der ausgewählten Taxonomie angezeigt werden.', 'upfront')
			);
			...
			foreach ( $this->inputs as $tab_id => $inputs ){
				foreach ( $this->inputs[$tab_id] as $input_id => $input_options ){
					if ( !upfront_get('callback', $this->inputs[$tab_id][$input_id]) ){
				...
				foreach ($custom_fields as $post_type => $fields) {
					...
					foreach ($fields as $field_name => $posts_total) {
						...
						$inputs[$name] = array(
							...
							'tooltip' => 'Wähle dies um die Show zuzulassen ' . $field_name,
							'default' => false,
							'toggle'    => array(
								'false' => array(
									'hide' => array(
		...
		public static function get_tax_terms($taxonomies, $keys_only = false) {
			...
			foreach ( $taxonomies as $key => $tax_name ) {
				...
				if ( !$keys_only && count($terms[$key]) == 0 ) {
					$terms[$key]['none'] = __('Für diese Taxonomie wurden keine Begriffe gefunden', 'upfront');
				}
	...
	function pu_pin_board_block_prevent_404($status) {
		...
		if ( strpos($status, '404') && get_query_var('paged') && upfront_get('pb') )
