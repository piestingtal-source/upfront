...
class UpFrontContentBlockDisplay {
	...
	function get_setting($setting, $default = null) {
		...
		return UpFrontBlockAPI::get_setting($this->block, $setting, $default);
	...
	function display($args = array()) {
		...
		/* Populate JS variable with wp_query global that way loading the block content with AJAX still shows the correct content */
			if ( UpFrontRoute::is_visual_editor_iframe() && $this->get_setting('mode', 'default') == 'default' ) {

				echo '<script type="text/javascript">';
				echo 'UPFRONT_WP_Query_Vars = ' . json_encode($GLOBALS['wp_query']->query_vars) . ';';			
		...
		/* Display the 404 text if it's a 404 (has to be default behavior) */
			if ( is_404() && $this->get_setting('mode', 'default') == 'default' && !upfront_get('ve-live-content-query', $this->block, false) ) {
				$this->remove_hooks();
			...
			if ($this->get_setting('show-entry', true) || $this->get_setting('comments-visibility') != "hide" ) {
				...
				if ( !$have_posts && ( $this->query instanceof SWP_Query || ( is_search() && $this->get_setting( 'mode', 'default' ) == 'default' ) ) ) {
					...
					echo '<div class="entry-content">';
						echo apply_filters('upfront_search_no_results', __('<p>Leider gab es keinen Inhalt, der Deiner Suche entsprach.</p>', 'upfront'));
					echo '</div>';
	...
	function show_query_title() {
		...
		if ( $this->query instanceof SWP_Query ){
			...
			$return .= apply_filters( 'upfront_search_title', sprintf( __( $swp_engine_info['searcbtp_engine_label'] . ' Ergebnisse für: %s', 'upfront' ), '<span>' . $swp_search . '</span>' ) );
			$return .= '</h1>';

			echo apply_filters( 'upfront_query_title', $return );
			return;
		...
		if ( is_date() ) {
			...
			$return .= '<h1 class="archive-title date-archive-title">';
				...
				if ( is_day() )
					$return .= apply_filters('upfront_archive_title', sprintf( __( 'Tagesarchiv: %s', 'upfront' ), '<span>' . get_the_date() . '</span>'));

				elseif ( is_month() )
					$return .= apply_filters('upfront_archive_title', sprintf( __( 'Monatliche Archive: %s', 'upfront' ), '<span>' . get_the_date('F Y') . '</span>'));

				elseif ( is_year() )
					$return .= apply_filters('upfront_archive_title', sprintf( __( 'Jahresarchiv: %s', 'upfront' ), '<span>' . get_the_date('Y') . '</span>' ));

				else 
					$return .= apply_filters('upfront_archive_title', __( 'Blog Archiv', 'upfront'));
		...
		/* Kategorie Archive */
			...
			if( $this->get_setting('show-archive-title-type', 'normal') == 'normal'){
				...
				$return .= apply_filters('upfront_category_title', sprintf(__('Archiv der Kategorie: %s', 'upfront'), '<span>' . single_cat_title('', false) . '</span>'));


			}elseif ( $this->get_setting('show-archive-title-type') == 'only-archive-name' ) {			

				$return .= apply_filters('upfront_category_title', '<span>' . single_cat_title('', false) . '</span>');


			}elseif ( $this->get_setting('show-archive-title-type') == 'show-custom-archive-title' ) {


				$custom_title = $this->get_setting('custom-archive-title','Kategorie Archive');
				$custom_title = str_replace('%archive%', '<span>' . single_cat_title('', false) . '</span>', $custom_title);
				$return .= apply_filters('upfront_category_title', $custom_title);
			...
			if ( !empty($category_description) ){
				$return .= apply_filters('upfront_category_archive_meta', '<div class="archive-meta category-archive-meta">' . $category_description . '</div>');
			}
		...
		/* Author Archives */
			...
			$return .= '<h1 class="archive-title author-title">';
				...
				if ( strpos($author_url, 'http') === 0 )
					$return .= sprintf(__( 'Autorenarchiv: %s', 'upfront'), '<span class="vcard"><a class="url fn n" href="' . $author_url . '" title="' . esc_attr($author->display_name) . '" rel="author">' . $author->display_name . '</a></span>');

				else
					$return .= sprintf(__( 'Autorenarchiv: %s', 'upfront'), '<span class="vcard">' . $author->display_name . '</span>');
		...
		/* Search */
			...
			$return .= '<h1 class="archive-title search-title">';
				$return .= apply_filters('upfront_search_title', sprintf(__('Suchergebnisse für: %s', 'upfront'), '<span>' . get_search_query() . '</span>'));
			$return .= '</h1>';
		...
		/* Tag Archives */
			...
			$return .= '<h1 class="archive-title search-title">';
				$return .= apply_filters('upfront_tag_title', sprintf(__('Tag-Archiv: %s', 'upfront'), '<span>' . single_tag_title('', false) . '</span>'));
			$return .= '</h1>';

			$tag_description = tag_description();
			if ( !empty($tag_description) )
				$return .= apply_filters('upfront_tag_archive_meta', '<div class="archive-meta tag-archive-meta">' . $tag_description . '</div>');
		...
		/* Custom Post Type Archives */
			...
			$return .= '<h1 class="archive-title post-type-archive-title">';
				$return .= apply_filters('upfront_post_type_archive_title', $queried_object->labels->name);
			$return .= '</h1>';
		...
		/* Custom Taxonomy Archives */
			...
			$return .= '<h1 class="archive-title taxonomy-archive-title">';
				$return .= apply_filters('upfront_taxonomy_archive_title', $taxonomy->labels->singular_name . ': <span>' . $term->name . '</span>');
			$return .= '</h1>';

			$term_description = term_description();
			if ( !empty($term_description) )
				$return .= apply_filters('upfront_term_archive_meta', '<div class="archive-meta term-archive-meta">' . $term_description . '</div>');		
	...
	function setup_query() {
		...
		if ( $this->get_setting('mode', 'default') == 'default' ) {
			...
			if ( upfront_post( 'wpQueryVars' ) && is_array( upfront_post( 'wpQueryVars' ) ) ) {


				$query_options = upfront_post( 'wpQueryVars' );
		...
		if ( $this->get_setting('show-entry', true) ) {
			...
			/* Meta */
				...
				if ( is_array($entry_meta_display_post_types) && in_array($post_type, $entry_meta_display_post_types) ) {
					...
					$entry_meta_above = $this->parse_meta($this->get_setting('entry-meta-above', 'Veröffentlicht am %date% von %author% &bull; %comments%'));
					$entry_utility_below = $this->parse_meta($this->get_setting('entry-utility-below', 'Abgelegt unter: %categories%'));

					if ( $entry_meta_above )
						$entry_meta_above = '<div class="entry-meta entry-meta-above">' . upfront_parse_php($entry_meta_above) . '</div>';

					if ( $entry_utility_below )
						$entry_utility_below = '<footer class="entry-utility entry-utility-below entry-meta">' . upfront_parse_php($entry_utility_below) . '</footer>';
			...
			/* Setup Titles */
				$hide_title = UpFrontLayoutOption::get($post_id, 'hide-title', false, true, false);
				$alternate_title = UpFrontLayoutOption::get($post_id, 'alternate-title', false, true);

				$post_title = (isset($alternate_title) && $alternate_title) ? $alternate_title : get_the_title();
				$post_title_tooltip = sprintf(esc_attr__('%s', 'upfront'), the_title_attribute('echo=0'));
			...
			do_action('upfront_before_entry', $args);

            $schema_itemtype = $post_type == 'post' ? 'Article' : 'CreativeWork';

			if(	$this->get_setting('featured-image-as-background', false)){

				$featured_image = apply_filters('upfront_featured_image_src', wp_get_attachment_image_src(get_post_thumbnail_id($post_id), 'full'));
				$featured_image_url = apply_filters('upfront_featured_image_url', $featured_image[0]);
			...
			if(isset($this->block['custom-fields']['above']) && is_array($this->block['custom-fields']['above']) && count($this->block['custom-fields']['above'])>0){
				...
				echo '<div class="'. implode(' ', apply_filters('upfront_content_custom_fields_class', array('custom-fields', 'custom-fields-above') ) )  .'">';

				foreach ($this->block['custom-fields']['above'] as $post_type => $custom_fields) {

					foreach ($custom_fields as $field_name => $label) {

						$group_tag = apply_filters('upfront_content_custom_fields_group_tag', 'div' );
						$label_tag = apply_filters('upfront_content_custom_fields_label_tag', 'label' );
						$field_tag = apply_filters('upfront_content_custom_fields_field_tag', 'div' );

						$custom_field_content = get_post_meta($post_id,$field_name,true);
						$custom_field_content = apply_filters('upfront_content_custom_fields_field_content', $custom_field_content );
			...
			echo UpFrontSchema::article($post);



			echo '<link itemprop="mainEntityOfPage" href="'.get_permalink($post_id).'" />';

					do_action('upfront_entry_open', $args);		
					...
					if ( $this->get_setting( 'show-titles', true ) || $entry_meta_above ) {
						...
						do_action('upfront_before_entry_title', $args);			
						...
						if (
							...
							echo '<' . $title_tag . ' class="entry-title" itemprop="headline">';
								...
								if ( apply_filters('upfront_show_edit_link', $this->get_setting('show-edit-link', true)) )
						...
						do_action('upfront_after_entry_title', $args);


						/**
						 *
						 * Custom Fields "Nach dem Titel"
						 *
						 */
						if(isset($this->block['custom-fields']['after-title']) && is_array($this->block['custom-fields']['after-title']) && count($this->block['custom-fields']['after-title'])>0){
							...
							echo '<div class="'. implode(' ', apply_filters('upfront_content_custom_fields_class', array('custom-fields', 'custom-fields-after-title') ) )  .'">';
					...
					do_action('upfront_entry_close', $args);			
			...
			if(isset($this->block['custom-fields']['below']) && is_array($this->block['custom-fields']['below']) && count($this->block['custom-fields']['below'])>0){
				...
				echo '<div class="'. implode(' ', apply_filters('upfront_content_custom_fields_class', array('custom-fields', 'custom-fields-below') ) )  .'">';
				...
				do_action('upfront_after_entry', $args);
	...
	function display_entry_content($args) {
		...
		do_action('upfront_before_entry_content', $args);
		...
		if ( $show_full_entries || get_post_type() == 'forum' ) {
			...
			echo '<div class="entry-content" itemprop="text">';
				...
				wp_link_pages(array( 'before' => '<div class="page-link">' . __( 'Seiten:', 'upfront' ), 'after' => '</div>' ));
		...
		do_action('upfront_after_entry_content', $args);
	...
	function display_404() {
		...
		echo '<div id="post-' . $post_id . '" class="' . $post_class . '">';
			...
			do_action('upfront_before_entry_title', $args);			
				...
				echo '<h1 class="entry-title">' . __('Hoppla! Seite nicht gefunden', 'upfront') . '</h1>';
			...
			do_action('upfront_before_entry_content', $args);			
				...
				echo '<div class="entry-content">';
					...
					echo __('<p>Ärgere Dich nicht, Du hast nichts falsch gemacht. Es scheint, dass die gesuchte Seite nicht existiert oder an eine andere Stelle verschoben wurde.</p>', 'upfront');

					echo sprintf(__('<p>Wenn Du weiterhin hier landest, kehre bitte zu unserer <a href="%s">Startseite</a> zurück oder versuche es mit dem unten stehenden Suchformular.</p>', 'upfront'), home_url());
	...
	function display_comments($hook_args) {
		...
		add_filter('upfront_comment_form_args', array($this, 'modify_comment_args'));		
		...
		do_action('upfront_before_entry_comments', $hook_args);		

		$withcomments = true;

		/* Display UpFront comments and send args to the comments_template() via a global variable */
		$upfront_comments_template_args = array(
			'comments-area-heading-responses-number-1' => $this->get_setting('comments-area-heading-responses-number-1', 'Eine Antwort'),
			'comments-area-heading-responses-number' => $this->get_setting('comments-area-heading-responses-number', '%num% Antworten'),
			'comments-area-heading' => $this->get_setting('comments-area-heading', '%responses% to <em>%title%</em>')
		);

		comments_template();

		do_action('upfront_after_entry_comments', $hook_args);		
		...
		function modify_comment_args() {
			...
			$leave_reply = $this->get_setting('leave-reply', 'Hinterlasse ein Kommentar');
			$title_reply_to = $this->get_setting('leave-reply-to', 'Hinterlasse ein Kommentar zu %s');
			$cancel_reply_text = $this->get_setting('cancel-reply-link', 'Kommentar verwerfen');
			$submit_text = $this->get_setting('label-submit-text', 'Kommentar hinzufügen');

			$comments_args = array(
				'comment_notes_before' => null,
				'comment_notes_after' => null,
				'cancel_reply_link' => __('Kommentar verwerfen', 'upfront'),
	...
	function display_pagination($position = 'below') {
		...
		echo '<div id="nav-' . $position . '" class="loop-navigation loop-utility loop-utility-' . $position . '" itemscope itemtype="http://schema.org/SiteNavigationElement">';
			...
			if ( $this->query instanceof SWP_Query ) {
				...
				$older_posts_text = __('<span class="meta-nav">&larr;</span> Ältere Beiträge', 'upfront');
				$newer_posts_text = __('Neuere Beiträge <span class="meta-nav">&rarr;</span>', 'upfront');
	...
	function display_thumbnail($post, $area = 'above-title') {
		...
		if ( !has_post_thumbnail() || !$this->get_setting('show-post-thumbnails', true) || !apply_filters('upfront_featured_image_src', wp_get_attachment_image_src(get_post_thumbnail_id(), 'full')) || $this->get_setting('featured-image-as-background', false))
			return;

		$entry_thumbnail_position = $this->get_setting('use-entry-thumbnail-position', true) ? UpFrontLayoutOption::get($post->ID, 'position', null, true, 'post-thumbnail') : false;
		...
		/* Get the size for cropping */
			if ( $position == 'left' || $position == 'right'  || $position == 'left-content' || $position == 'right-content' )  {
				...
				$thumbnail_width = ( $this->get_setting( 'enable-column-layout' ) && ! ( is_singular() && $this->get_setting( 'mode', 'default' ) == 'default' ) ) ? UpFrontContentBlock::get_column_width($this->block) : UpFrontBlocksData::get_block_width($this->block);
		...
		/* Get the image URL */
			if ( $this->get_setting('crop-post-thumbnails', true) ) {
				...
				$thumbnail = apply_filters('upfront_featured_image_src', wp_get_attachment_image_src(get_post_thumbnail_id(), 'full'));  

				$thumbnail_url = apply_filters('upfront_featured_image_url', upfront_resize_image($thumbnail[0], $thumbnail_width, $thumbnail_height));

			} else {

				$thumbnail = apply_filters('upfront_featured_image_src', wp_get_attachment_image_src(get_post_thumbnail_id(), array(
					$thumbnail_width, 
					$thumbnail_height
				)));  

				$thumbnail_url = apply_filters('upfront_featured_image_url', $thumbnail[0]);
	...
	function parse_meta($meta) {
		...
		foreach ( $variables as $variable ) {
			...
			switch ( $variable ) {
				...
				case 'comments_no_link':
					...
					$replacement['comments'] = '<a href="' . get_comments_link() . '" title="' . sprintf(__('%s &ndash; Kommentare', 'upfront'), the_title_attribute('echo=0')) . '" class="entry-comments">' . $comments . '</a>';
				...
				case 'respond':
					...
					$replacement['respond'] = '<a href="' . get_permalink() . '#respond" title="' . sprintf(__('Reagiere auf %s', 'upfront'), the_title_attribute('echo=0')) . '" class="entry-respond">' . $respond_format . '</a>';
				...
				case 'author':
					...
					$replacement['author'] .= '<a class="author-link fn nickname url" href="' . get_author_posts_url($authordata->ID) . '" title="' . sprintf(__('Alle Beiträge von %s', 'upfront'), $authordata->display_name) . '">';
		...
		return apply_filters('upfront_meta', $meta);
	...
	function entry_class() {
		...
		if ( $custom_css_class = UpFrontLayoutOption::get(get_the_id(), 'css-class', null, true) ) {
	...
	function more_link($more_link = null) {
		...
		return apply_filters('upfront_more_link', ' ' . $more_link);
	...
	function filter_nofollow_links_in_post($text) {
		...
		if ( !is_object($post) || empty($post->ID) || !UpFrontSEO::is_seo_checkbox_enabled('nofollow', $post->ID) )
			return $text;
