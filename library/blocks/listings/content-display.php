<?php
class UpFrontListingBlockDisplay {
	...
	function display($args = array()) {
		...
		$listing_type = UpFrontBlockAPI::get_setting($block, 'listing-type', 'taxonomy');
	...
	function display_taxonomy($args = array()) {
		...
		$taxonomy = UpFrontBlockAPI::get_setting($block, 'select-taxonomy', 'category');

		$args = array(
		    'orderby'       => UpFrontBlockAPI::get_setting($block, 'terms-orderby', 'name'), 
		    'order'         => UpFrontBlockAPI::get_setting($block, 'terms-order', 'ASC'),
		    'hide_empty'    => UpFrontBlockAPI::get_setting($block, 'terms-hide-empty', 0), 
		    'exclude'       => UpFrontBlockAPI::get_setting($block, 'terms-exclude', array()),
		    'exclude_tree'  => array(), 
		    'include'       => UpFrontBlockAPI::get_setting($block, 'terms-include', array()),
		    'number'        => UpFrontBlockAPI::get_setting($block, 'terms-number', '10'), 
		    'fields'        => 'all', 
		    'slug'          => UpFrontBlockAPI::get_setting($block, 'terms-slug', ''), 
		    'parent'        => '',
		    'hierarchical'  => UpFrontBlockAPI::get_setting($block, 'terms-hierarchical', true), 
		    'child_of'      => 0, 
		    'get'           => '', 
		    'name__like'    => '',
		    'pad_counts'    => false, 
	...
	function display_entry($args = array()) {
		...
		/* Setup Titles */
			$hide_title = UpFrontLayoutOption::get($post_id, 'hide-title', false, true);
