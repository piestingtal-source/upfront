...
class UpFrontBlocks {
	...
	public static function init() {
		...
		add_action('upfront_register_elements_instances', array(__CLASS__, 'register_block_element_instances'), 11);

		add_action('upfront_block_content_unknown', array(__CLASS__, 'unknown_block_content'));

		/* Clear the block actions cache upon Visual Editor save and others */
		add_action('upfront_flush_cache', array(__CLASS__, 'clear_block_actions_cache'));
	...
	public static function register_block_types() {
		...
		global $upfront_unregistered_block_types;
		global $upfront_registry;

		foreach ( $upfront_unregistered_block_types as $class => $block_type_data ) {

			if( is_array($block_type_data) ){

				// Add this class and path to global class upfront_registry to be loaded in loader.php
				$upfront_registry = array_merge($upfront_registry, array($class => $block_type_data['block_type_path']));

				if ( !class_exists($class) )
					return new WP_Error('block_class_does_not_exist', __('The block class being registered does not exist.', 'upfront'), $class);
				...
				// Before UpFront 1.2.0: block_type_url = block_type_data
				if ( $block_type_data )
		...
		unset($upfront_unregistered_block_types);
		return true;
	...
	public static function process_registered_blocks() {
		...
		do_action('upfront_register_blocks');
	...
	public static function load_core_blocks() {
		...
		foreach ( apply_filters('upfront_core_block_types', self::$core_blocks) as $block ) {

			$block_path = '/blocks/' . $block . '/' . $block . '.php';

			/* Allow blocks to be overriden by child themes */
			if ( UPFRONT_CHILD_THEME_ACTIVE && file_exists( untrailingslashit(UPFRONT_CHILD_THEME_DIR) . $block_path ) )
				require_once untrailingslashit( UPFRONT_CHILD_THEME_DIR ) . $block_path;
			else
				require_once UPFRONT_LIBRARY_DIR . $block_path;
	...
	public static function register_core_blocks(){
		...
		foreach ( apply_filters('upfront_core_block_types', self::$core_blocks) as $block ) {
			...
			/* Allow blocks to be overriden by child themes */
			if ( UPFRONT_CHILD_THEME_ACTIVE && file_exists( untrailingslashit(UPFRONT_CHILD_THEME_DIR) . $register_file ) )
				require_once untrailingslashit( UPFRONT_CHILD_THEME_DIR ) . $register_file;
			else
				require_once UPFRONT_LIBRARY_DIR . $register_file;
	...
	public static function setup_block_actions() {
		...
		/* If cache exists then use it */
			if ( ($block_actions_transient = get_transient('pu_block_actions_template_' . UpFrontOption::$current_skin)) && !(upfront_get('ve-iframe') && UpFrontCapabilities::can_user_visually_edit()) ) {
		...
		/* Build the cache */
			...
			foreach ( $block_types as $block_type => $block_type_options ) {
				...
				foreach ( $blocks as $block_id => $block ) {
					...
					if ( !UpFrontLayout::is_customized($layout_id) && strpos($layout_id, 'template-') === false )
						continue;
					...
					/* Add block to Block Objects Array */
						...
						if ( !upfront_get($block_id, self::$block_actions['block-objects']) ) {
		...
		/* Pull block actions from blocks in mirrored wrappers */
			$mirrored_wrappers = UpFrontWrappersData::get_all_wrappers(false, true);
		...
		/* Set the cache */
			set_transient('pu_block_actions_template_' . UpFrontOption::$current_skin, self::$block_actions);
		...
		public static function clear_block_actions_cache() {
			...
			return delete_transient('pu_block_actions_template_' . UpFrontOption::$current_skin);
	...
	public static function get_block_actions_from_mirrored_wrappers($wrappers, $block_action) {
		...
		foreach ( $wrappers as $wrapper_id => $wrapper ) {
			...
			foreach ( upfront_get($wrapper_being_mirrored['layout'], self::$block_actions[$block_action], array()) as $block_id_on_layout_of_mirrored_wrapper ) {
				...
				if ( upfront_get('wrapper_id', upfront_get($block_id_on_layout_of_mirrored_wrapper, self::$block_actions['block-objects'])) == $wrapper_being_mirrored['id'] ) {
	...
	public static function run_block_init_actions() {
		...
		foreach ( self::$block_actions['init'] as $layout_id => $blocks ) {
			...
			foreach ( $blocks as $block_id ) {
				...
				$block_options = upfront_get($block_id, self::$block_actions['block-objects']);

				/* Use legacy ID if present */
				$block_id            = UpFrontBlocksData::get_legacy_id($block_options);
	...
	public static function run_block_enqueue_actions() {
		...
		if ( is_admin() || UpFrontRoute::is_visual_editor() )
			return false;

		$layout_id = UpFrontLayout::get_current_in_use();

		$enqueue_action_blocks = upfront_get($layout_id, self::$block_actions['enqueue'], array());
		...
		foreach ( $enqueue_action_blocks as $block_id ) {
			...
			/* If the block is mirrored, then use that ID instead */
				if ( $possible_mirror_id = UpFrontBlocksData::is_block_mirrored($block_options) ) {
			...
			if ( is_callable(array(upfront_get('class', $block_options), 'enqueue_action')) )
				call_user_func(array(upfront_get('class', $block_options), 'enqueue_action'), $block_id, $block_options, $original_block);
	...
	/* Functions used for Dynamic JS and Dynamic CSS */
		...
		public static function get_blocks_for_dynamic_asset($css_or_js, $layout_id) {
			...
			return upfront_get($layout_id, self::$block_actions['dynamic-' . $css_or_js], array());
			...
			foreach ( $blocks as $block_id ) {
				...
				/* If the block is mirrored, then use that ID instead */
					if ( $possible_mirror_id = UpFrontBlocksData::is_block_mirrored($block_options) ) {
						...
						/* Use legacy ID if present */
						$original_block['id'] = UpFrontBlocksData::get_legacy_id( $original_block );
				...
				/* Output the CSS or JS */
					switch ( $css_or_js ) {
						...
						case 'js':
							if ( is_callable(array(upfront_get('class', $block_options), 'dynamic_js')) )
								$data .= call_user_func(array(upfront_get('class', $block_options), 'dynamic_js'), $block_id, $block_options, $original_block);
							elseif ( is_callable(array(upfront_get('class', $block_options), 'js_content')) )
								$data .= call_user_func(array(upfront_get('class', $block_options), 'js_content'), $block_id, $block_options, $original_block);
						break;

						case 'css':
							if ( is_callable(array(upfront_get('class', $block_options), 'dynamic_css')) )
								$data .= call_user_func(array(upfront_get('class', $block_options), 'dynamic_css'), $block_id, $block_options, $original_block);
						break;
	...
	/* Dynamic JS */
		public static function output_block_dynamic_js($layout_id = false) {
			...
			$layout_id = !$layout_id ? upfront_get('layout-in-use') : $layout_id;
		...
		public static function enqueue_block_dynamic_js_file() {
			...
			$script_name 			= str_replace(UpFrontLayout::$sep, '-', 'block-dynamic-js-layout-' . UpFrontLayout::get_current_in_use());			
			...
			UpFrontCompiler::register_file(array(
				...
				'format' => 'js',
				'fragments' => array(
					array('UpFrontBlocks', 'output_block_dynamic_js')
				),
				'enqueue' => false
			));

			if ( strlen((string)self::output_block_dynamic_js($current_layout_in_use)) > 0 )
				wp_enqueue_script($script_name, UpFrontCompiler::get_url($script_name), array('jquery'));
	...
	public static function register_block_element_instances() {
		...
		if ( !($blocks = UpFrontBlocksData::get_all_blocks()) )
			return false;
		...
		foreach ( $blocks as $block ) {
			...
			if ( $block_mirror = UpFrontBlocksData::is_block_mirrored($block) )
				continue;

			$default_name = self::block_type_nice($block['type']) . ' (Unbenannt)';
			$name = upfront_get('alias', $block['settings']) ? upfront_get('alias', $block['settings']) : $default_name;

			$block_id_for_selector = UpFrontBlocksData::get_legacy_id( $block );
			...
			UpFrontElementAPI::register_element_instance(array(
				...
				foreach ( UpFrontElementAPI::get_block_elements($block['type']) as $block_element_sub_element ) {

					/* Make sure that the element supports instances */
					if ( !upfront_get('supports-instances', $block_element_sub_element) )
						continue;
	...
	public static function display_block($block, $where = null) {
		...
		if ( !is_array($block) )
			$block = UpFrontBlocksData::get_block($block);
		...
		if ( !$block_type_settings = upfront_get($block['type'], $block_types, array()) ) {
		...
		$custom_css_classes = str_replace('  ', ' ', str_replace(',', ' ', esc_attr(strip_tags(upfront_get('css-classes', $block['settings'], '')))));
		...
		$block_classes[] = ( upfront_get('fixed-height', $block_type_settings, false) !== true ) ? 'block-fluid-height' : 'block-fixed-height';
		...
		if ( UpFrontResponsiveGrid::is_enabled() ) {
			...
			$responsive_block_hiding = upfront_get('responsive-block-hiding', $block['settings'], array());
		...
		if ( $mirrored_block = UpFrontBlocksData::get_block_mirror($block) ) {
			...
			if ( $where != 'grid' ) {
				...
				$block_classes[] = 'block-original-' . UpFrontBlocksData::get_legacy_id( $block['original'] );
		...
		$block_tag = ( $html_tag = upfront_get('html-tag', $block_type_settings) ) ? $html_tag : 'div';

		//The ID attribute for the block.  This will change if mirrored.
		$block_id_for_id_attr = $where != 'grid' ? UpFrontBlocksData::get_legacy_id( $block ) : $block['id'];
		...
		if ( UpFrontRoute::is_visual_editor_iframe() ) {
			...
			$block_data_attrs = implode(' ', array(
				...
				'data-type="' . upfront_get('type', $block) . '"',
				...
				'data-alias="' . esc_attr(stripslashes(upfront_get('alias', upfront_get('settings', $block, array())))) . '"',
				'data-custom-classes="' . trim($custom_css_classes) . '"',
				'data-inline-editable="' . upfront_get('inline-editable', $block_type_settings) . '"'
			));

		} else {

			$block_data_attrs = implode( ' ', array(
				' data-alias="' . esc_attr(upfront_get( 'alias', upfront_get( 'settings', $block, array() ) )) . '"'
			) );
		...
		if ( $where != 'grid' ) {
			...
			do_action('upfront_before_block', $block);
			do_action('upfront_before_block_' . $block['id'], $block);

			echo '<' . $block_tag . ' id="block-' . $block_id_for_id_attr . '" class="' . implode(' ', array_filter(apply_filters('upfront_block_class', $block_classes, $block))) . '"' . $block_style_attr . $block_data_attrs . self::block_attr( $block_type_settings ) . '>';

				do_action('upfront_block_open', $block);
				do_action('upfront_block_open_' . $block['id'], $block);

				echo '<div class="block-content">';

					do_action('upfront_block_content_open', $block);
					do_action('upfront_block_content_open_' . $block['id'], $block);

					do_action('upfront_block_content_' . $block['type'], $block);

					do_action('upfront_block_content_close', $block);
					do_action('upfront_block_content_close_' . $block['id'], $block);

				echo '</div>';

				do_action('upfront_block_close', $block);
				do_action('upfront_block_close_' . $block['id'], $block);

			echo '</' . $block_tag . '>';

			do_action('upfront_after_block', $block);
			do_action('upfront_after_block_' . $block['id'], $block);

		//Show the block in the grid
		} else {

			$show_content_in_grid = self::block_type_exists($block['type']) ? upfront_get('show-content-in-grid', $block_type_settings, false) : false;
	...
	public static function block_attr( $block_type_settings ) {
		...
		if ( !$attributes = upfront_get('attributes', $block_type_settings) )
			return;
	...
	public static function get_block_types() {
		...
		global $upfront_block_types;		

		if ( !isset($upfront_block_types) || empty($upfront_block_types) )
			return null;

		return $upfront_block_types;
	...
	public static function get_registered_blocks_categories(){
		...
		$categories = array();
		foreach ($upfront_block_types as $block_type => $parameters) {
	...
	public static function block_type_nice($type) {
		...
		return upfront_get('name', upfront_get($type, $block_types));
	...
	public static function block_type_exists($type) {
		...
		if ( !is_array($block_types) )
			return new WP_Error('blocks_array_does_not_exist', __('The UpFront blocks array does not exist.', 'upfront'));
	...
	public static function unknown_block_content($block = null) {
		...
		echo '<div class="alert alert-red block-type-unknown-notice"><p>' . sprintf( __('The requested block type of \'%s\' does not exist. Please re-activate the block plugin or child theme if you wish to use this block again.', 'upfront'), $block['requested-type']) . '</p></div>';
