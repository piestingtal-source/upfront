...
New options for Google Schriftarten
- Option to set Font Display ( eg. swap )
- Optino to preload Google Schriftarten
...
Fixed: UpFrontElementAPI::register_element() minor fix, default properties missing
...
Fixed: UpFrontQuery::get_categories() change to convert parameter to array when parameter is not array
...
-- upfront_content_custom_fields_group_tag
-- upfront_content_custom_fields_label_tag
-- upfront_content_custom_fields_field_tag
-- upfront_content_custom_fields_field_content
-- upfront_content_custom_fields_class
New features on PinBoard Block
- Added option to exclude current post from PinBoard block when custom query is used
- Added: Custom Fields support, includes new filters:
-- upfront_pin_board_pin_custom_fields_group_tag
-- upfront_pin_board_pin_custom_fields_label_tag
-- upfront_pin_board_pin_custom_fields_field_tag
-- upfront_pin_board_pin_custom_fields_field_content
-- upfront_pin_board_pin_custom_fields_class
New Option to upgrade and use UpFront Edge version
New Block: Site Logo
New Block: Divider
New Block: OnePage Navigation
New inline editor for certain fields on Visual Editor
New features on UpFrontQuery
- UpFrontQuery::get_tags() now support custom taxonomy parameter
...
Added constant UPFRONT_DISABLE_PHP_VERIFICATION, if true UpFront will not verify PHP version (useful to migrate from old php version sites)
Added 'font-display: swap' to Google Schriftarten to ensure text remains visible during webfont load
Added option to load Google Schriftarten asynchronously
Added option to do not use Google Schriftarten
...
Added option to show UpFront Blocks as Gutenberg Blocks
Added Rel option for block title link
Added new animation options like "Fill Mode", "Play State" and "When animate"
Added new filter 'upfront_header_link' to Header Block, it allows to set the header link to override the default URL given by home_url() function
Updated Constant UPFRONT_DISABLE_PHP_PARSING is false by default
...
Fixed: Issue when showing templates saved on UpFront Services account
...
New option to prevent mirror Container styles
...
Visual Editor Fixes
- Fixed extra padding on both Gitter and Design View.
- LiveCSS fixes
...
Save templates on cloud with UpFront Services
...
Google Web Schriftarten Variant fixed
...
Web Schriftarten Library Updated
...
UpFront LifeSaver plugin compatibility
...
Updated "Gitter Wizard" to "Gitter Manager"
Content editor fixes
...
Fixed Google Web Schriftarten API
...
Integration of TGM-Plugin-Activation to recommends UpFront Services plugin
...
Renamed all references from Box to UpFront
